...
public class BusMoreActivity extends FragmentActivity  {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		showImage(new SharePreference(this).getValueForBool(SharePreference.BUS_SHARE_IS_SHOW));
		showSelectDate(new SharePreference(this).getValueForInt(SharePreference.BUS_SHARE_DEFAULT_DATE));
	...
	public void click(View v)
	{
		switch (v.getId())
		{
			case R.id.bus_more_about:
				...
				bundle.putInt(AboutActivity.ABOUT_ID, 2);
			...
			case R.id.bus_more_suggestion:
				...
				suggesiontBundle.putInt(SuggestionActivity.SUGGESTION_NAME, 2);
			...
			case R.id.bus_more_complain:
				...
				complainBundle.putInt(ComplainActivity.COMPLAIN_NAME, 2);
			...
			GetDialog.messageDialog(this, "提示", "确定删除历史记录？", "是", new OnClickListener()
			{
				...
				{
					HistoryDBOperate dbOperate=new HistoryDBOperate(BusMoreActivity.this);
					dbOperate.deleteAll(ContentType.BUS_STATION_SEARCH_HISROTY);	
					dbOperate.deleteAll(ContentType.BUS_STATION_NO_SEARCH_HISROTY);
					dbOperate.deleteAll(ContentType.BUS_STATION_STATION_SEARCH_HISROTY);
					dbOperate.CloseDB();
					
					Toast.makeText(BusMoreActivity.this, "历史记录清除成功", Toast.LENGTH_SHORT).show();
					dialog.cancel();
				}
			...
			case R.id.bus_more_select_date:
				...
				GetDialog.getRadioDialog(this, "出发日期", R.array.array_air_date, new OnItemSelectListener(), "", null, new SharePreference(this).getValueForInt(SharePreference.BUS_SHARE_DEFAULT_DATE)).show();
				
				break;
				
			case R.id.bus_more_select_show_type:
				SharePreference preference=new SharePreference(this);
				boolean b=preference.getValueForBool(SharePreference.BUS_SHARE_IS_SHOW);
				
				b=!b;
				preference.saveValeForBool(SharePreference.BUS_SHARE_IS_SHOW, b);
				showImage(b);
	...
	class OnItemSelectListener implements OnClickListener
	{
		...
		public void onClick(DialogInterface dialog, int which)
		{
			...
			new SharePreference(BusMoreActivity.this).saveValeForInt(SharePreference.BUS_SHARE_DEFAULT_DATE, which);
			dialog.cancel();
		}
