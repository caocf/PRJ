 ...
 * 
 */
public class MarkMapOverItem extends CNMKItemizedOverlay
{
	...
	private View pop;
	private List<MarkOverlayItem> points;
	private OnPopInterface popInterface;

	public MarkMapOverItem(Drawable marker, Context context,
			CNMKMapView mapView, View view, List<MarkOverlayItem> point,
			OnPopInterface popInterface)
	{
		...
		{
			CNMKOverlayItem temp = new CNMKOverlayItem(points.get(i)
					.getGeoPoint(), "", "");

			Drawable drawable = context.getResources().getDrawable(
					points.get(i).getMarkid());

			Rect rect0 = drawable.getBounds();
			if ((rect0.height() == 0) || (rect0.width() == 0))
				drawable.setBounds(0, 0, drawable.getIntrinsicWidth(),
						drawable.getIntrinsicHeight());

			Rect rect1 = drawable.getBounds();
			int li = rect1.width() / 2;
			int j = -rect1.height();
			int k = 0;
			...
			drawable.setBounds(-li, j, li, k);

			// marker.setBounds( -marker.getIntrinsicWidth(), 0,
	...
	protected boolean onTap(int i)
	{
		...
		if (popInterface != null && pop != null)
		{
			...
			mapView.updateViewLayout(pop, CNMKMapView.newLayoutParams(
					LayoutParams.WRAP_CONTENT, LayoutParams.WRAP_CONTENT, pt,
					CNMKMapView.LayoutParams_BOTTOM_CENTER));
