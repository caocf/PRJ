...
public class RegistSecondCoastal extends Activity
{
	...
	private void showImg(final int index)
	{
		...
		builder.setPositiveButton("删除", new DialogInterface.OnClickListener() 
		{
			...
			public void onClick(DialogInterface dialog, int which)
			{
				...
				files[index]=null;
				Drawable drawable=RegistSecondCoastal.this.getResources().getDrawable(R.drawable.registe_ic_camera);
		...
		{				
			...
			public void onClick(DialogInterface dialog, int which)
			{
				...
				Dialog d_choose=new Dialog(RegistSecondCoastal.this);
				d_choose.requestWindowFeature(Window.FEATURE_NO_TITLE);				
				
				ImageView imvImageView=new ImageView(RegistSecondCoastal.this);
	...
	public void onSubmitShip(View v)
	{
		HttpRequest hr=new HttpRequest(new HttpRequest.onResult() 
		{
			...
			public void onSuccess(String result) 
			{
				...
				Toast.makeText(RegistSecondCoastal.this, "资料提交成功，请等待审核", Toast.LENGTH_LONG).show();
				finish();
		...
		{
			Toast.makeText(RegistSecondCoastal.this, "请输入公司名！", Toast.LENGTH_LONG).show();
			return;
		}
		if("".equals( shipNum.getText().toString().trim()))
		{
			Toast.makeText(RegistSecondCoastal.this, "请输入营业执照号！", Toast.LENGTH_LONG).show();
			return;
		}
		if(files[0]==null)
		{
			Toast.makeText(RegistSecondCoastal.this, "请添加营业执照文件！", Toast.LENGTH_LONG).show();
			return;
		}
		
		Map<String, File> fileMap=new HashMap<>();
		fileMap.put("LS", files[0]);
		...
		pmap.put("company", shipName.getText().toString().trim());
		pmap.put("BLnumber", shipNum.getText().toString().trim());
		try {
			hr.post(contants.baseUrl+"register", pmap, fileMap, "");
	...
	public void ConfirmQite(View v)
	{
		...
		builder.setPositiveButton("退出", new DialogInterface.OnClickListener() 
		{
			...
			public void onClick(DialogInterface dialog, int which)
			{
				RegistSecondCoastal.this.finish();
			}
		});
