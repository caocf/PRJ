$(document).ready(/*function() {
	...
	$.ajax({
		...
		success : function(data) {
			...
			document.getElementById("title").innerHTML = "省港航公众版的所有版本";
			document.getElementById("apptitle").innerHTML = "省港航公众版的所有版本";
	...
	$.ajax({
		...
		success : function(data)
		{
			...
			for (var i = 0; i < list.length; i++)
			{
				...
				newTr.append($("<td>" + list[i].versionNum + "</td>"));
				newTr.append($("<td><a style='cursor: auto' onclick='viewAppVersion("+ list[i].id + ")'>" + list[i].verstionDec + "</a></td>"));
				newTr.append($("<td>" + list[i].uptime + "</td>"));

				newTr.append($("<td><p rows='3' cols='10' name='updatelog' id='updatelog'>"+list[i].log+"</p></td>"));
				newTr.append($("<td><a style='color:blue;text-decoration:underline' href='"+list[i].address+"'>下载</a>&nbsp;"));
				/*newTr.append($("<td>" + getupdatetype(dataObj[i].updatetype) + "</td>"));
				newTr.append($("<td>" + getFileName(dataObj[i].respath) + "</td>"));
...
function addAppVersion()
{
	code=prompt("请输入密码","");
	if(code=="1234")
		window.location.href="appmanager/addappversioninfo.jsp";
	else
	{
		alert("密码错误");
	}
