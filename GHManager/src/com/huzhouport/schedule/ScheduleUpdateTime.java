...
public class ScheduleUpdateTime extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		((TextView)findViewById(R.id.schedule_update_title)).setText("�༭����");
		createRemind();
	...
	class UploadDate extends AsyncTask<Void, Void, String> {
		...
		protected String doInBackground(Void... params) {
			...
			if (et_name.getText().length() == 0) {
				...
				UploadActivity.tool.addFile("�޸�"+et_name.getText()+"�Ļ���",actionUrl, paramsDate,files, "scheduleAttachment.af");
			}
			return null;
		}
	...
	public void getScheduleKind(String result) {
		...
		try {
			...
			Toast.makeText(ScheduleUpdateTime.this, "���ݻ�ȡʧ��", Toast.LENGTH_SHORT)
					.show();
			e.printStackTrace();
		}
	...
	class DeleteAttachment extends AsyncTask<Void, Void, String> {
		...
		protected void onPostExecute(String result) {
			...
			try {
				...
				String jsonObject0 = data.getString("ieventId");
				if (jsonObject0.equals("1"))
					Toast.makeText(ScheduleUpdateTime.this, "ɾ���ɹ�",
							Toast.LENGTH_SHORT).show();
				old_files.remove(at_name[wzpost]);
				creatGV();
			} catch (Exception e) {
				Toast.makeText(ScheduleUpdateTime.this, "��������", Toast.LENGTH_SHORT)
						.show();
				e.printStackTrace();
			}
