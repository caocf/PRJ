...
function QueryOriginBusLineByName(list, listlength,direct) {
	for ( var i = 0; i < listlength; i++) {
    	...
    	if(upListInfo.length!=undefined||upListInfo.length!=0){
			...
			html+="<div class='addTr_div_1' id='addTr_"+linenum+"_1'><label class='addTr_div_1_title' onclick=busRouteCoordinate('"+lineInfo.id+"',1)>"+
					lineInfo.name+"("+up_firstStation+"-"+up_lastStation+")</label><label class='addTr_div_1_link' onclick=Line_details('"+lineInfo.id+"','-1',1)>详情&raquo;</label></div>";
			...
			html+="<div class='addTr_div_3' id='addTr_"+linenum+"_3' ><label onclick=openDiv('"+linenum+"') >" +
					"<img id='img_"+linenum+"' src='WebSit/image/common/plus.png'/>途径站点</label>";
	...
	if(downListInfo.length!=undefined||downListInfo.length!=0){
		...
		html+="<div class='addTr_div_1' id='addTr_"+linenum+"_1'><label class='addTr_div_1_title' onclick=busRouteCoordinate('"+lineInfo.id+"',2)>"+
				lineInfo.name+"("+down_firstStation+"-"+down_firstStation+")</label><label class='addTr_div_1_link' onclick=Line_details('"+lineInfo.id+"','-1',1)>详情&raquo;</label></div>";
...
function addPointPoup(lon, lat, stationId,lineName,lineId,direct,imageKind) {
	...
	var icon;
	if(imageKind==0){
		icon = map.createIcon("WebSit/image/common/map_point_start.png",36);
	}else if(imageKind==2){
		icon = map.createIcon("WebSit/image/common/map_point_end.png",36);
	}
	else{
		icon = map.createIcon("WebSit/image/common/map_circle.png",11);
	}
...
function Line_details(routeId,busStationId,direct){
	window.open($$("#basePath").val()+"WebSit/page/publicTraffic/wisdom/line_infor.jsp?routeId="+routeId+"&busStationId="+busStationId+"&direct="+direct);
}
...
function openDiv(num){
	...
	if (image.indexOf("plus", 0) != -1) {
		...
		$$("#img_"+num).attr("src","WebSit/image/common/minus.png");
	} else {
		$$("#addTr_"+num+"_4").css("display","none");
		$$("#img_"+num).attr("src","WebSit/image/common/plus.png");
	}
