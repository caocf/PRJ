...
document.onkeydown = function(event) {
	...
	if (e && e.keyCode == 13&&(e.srcElement.id=="startPoint"||e.srcElement.id=="endPoint")) { // enter 键
		SearchBusTransfer();
	}
};
//搜索
...
function appendToTable(list, listLength){
	...
	for ( var i = 0; i < listLength; i++) {
		...
		//起点标志
		newTr.append($$("<div class='addDiv_div_ends'><img src='WebPage/image/graphical/icon_origin.png'/><label>"+startLocation+"</label>" +
				"<img  class='addTr_div_1_colloct' src='WebPage/image/common/collect.png' onclick='SaveTransferFavor()' /></div></div>"));
		...
		//终点标志
		newTr.append($$("<div class='addDiv_div_ends'><img src='WebPage/image/graphical/icon_destenation.png'/><label>"+$$("#endPoint").val()+"</label></div>"));
		newDiv.append(newTr);
...
function busRouteCoordinate(num) {
	...
	var center=((Number(startpointlonlat.split(",")[0])+Number(endpointlonlat.split(",")[0]))/2).toFixed(6)+","+((Number(startpointlonlat.split(",")[1])+Number(endpointlonlat.split(",")[1]))/2).toFixed(6);
	map.setMapCenter(center,11);
