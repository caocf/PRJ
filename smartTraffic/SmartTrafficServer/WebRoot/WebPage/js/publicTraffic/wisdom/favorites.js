...
function appendToTable(list,num,transferFavors){
	...
	for ( var i = 0; i < list.length; i++) {
		if(list[i].LineList[0]!=null){
			...
			newTr_top.append($$("<div class='addDiv_top_div_1' ><label class='addDiv_top_div_num'>"+(i+1)+"</label><label class='addDiv_top_div_name'>"+list[i].LineList[0]+
			...
			var newTr = $$("<div class='addDiv_content' id='"+num+"' style='display:none'></div>");
			//起点标志
	...
	$$(".addDiv").click(function(){
		$$(".addDiv_content").hide();
		$$(this).children(".addDiv_content").show();
		routeTransfer($$(this).children(".addDiv_content").attr("id"));
	});
}
//地图显示整条线路
