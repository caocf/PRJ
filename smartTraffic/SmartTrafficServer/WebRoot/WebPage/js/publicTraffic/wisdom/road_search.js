...
document.onkeydown = function(event) {
	...
	if (e && e.keyCode == 13 && e.srcElement.id == "searchContent") { // enter
																		// 键
		queryBusRoute();
	}
};
...
function QueryOriginBusLineByName(list, listlength, direct) {
	for ( var i = 0; i < listlength; i++) {
		...
		var isRing=lineInfo.isRing;
		
		// 上行
		if (upListInfo.length != undefined || upListInfo.length != 0) {

			var html = "<div class='addTr' id='addTr_" + linenum + "'>";
			html += "<div class='addTr_div_1' id='addTr_"
					+ linenum
					+ "_1'><label class='addTr_div_1_title' onclick=busRouteCoordinate('"
					+ lineInfo.id
					+ "',1)>"
					+ lineInfo.name
					+ "("
					...
					+ "-"
					+ up_lastStation
					+ ")</label><label class='addTr_div_1_link' onclick=Line_details('"
					+ lineInfo.id
					+ "','-1',1)>详情&raquo;</label>"
					+ "<img  class='addTr_div_1_colloct' src='WebPage/image/common/collect.png' onclick=SaveStationFavor(this,'"
					+ lineInfo.id + "',1) /></div>";
			html += "<div class='addTr_div_2' id='addTr_" + linenum
					+ "_2'><label>首末班时间：" + lineInfo.lineUpStartTime + "-"
					+ lineInfo.lineUpEndTime + "&nbsp;全程：" + lineInfo.price
					+ "元</label></div>";
			html += "<div class='addTr_div_3' id='addTr_" + linenum
					+ "_3' ><label onclick=openDiv('" + linenum + "') >"
					+ "<img id='img_" + linenum
					+ "' src='WebPage/image/common/plus.png'/>途径站点</label>";
			html += "<div class='addTr_div_4' id='addTr_" + linenum
					+ "_4' style='display:none'>";

			for ( var j = 0; j < upListInfo.length; j++) {
				html += "<label onclick=busStationPoup('" + lineInfo.id + "','"
						+ upListInfo[j].stationId + "',1) >" + (j + 1) + "."
						+ upListInfo[j].stationName + "</label>";
			}

			html += "</div></div></div>";
			$$(".layout1_left_datalist").html(
					$$(".layout1_left_datalist").html() + html);
			linenum++;
		}
		...
		if (downListInfo.length != undefined || downListInfo.length != 0) {
			...
			html += "<div class='addTr_div_1' id='addTr_"
					...
					+ lineInfo.id + "',2) /></div>";
			html += "<div class='addTr_div_2' id='addTr_" + linenum
					+ "_2'><label>首末班时间：" + lineInfo.lineDownStartTime + "-"
					+ lineInfo.lineDownEndTime + "&nbsp;全程：" + lineInfo.price
					+ "元</label></div>";
			html += "<div class='addTr_div_3' id='addTr_" + linenum
					+ "_3' ><label onclick=openDiv('" + linenum + "') >"
					+ "<img id='img_" + linenum
			...
			for ( var j = 0; j < downListInfo.length; j++) {
				html += "<label onclick=busStationPoup('" + lineInfo.id + "','"
						+ downListInfo[j].stationId + "',2) >" + (j + 1) + "."
						+ downListInfo[j].stationName + "</label>";
			}
			...
			if (list != undefined) {
				if (direct == 1) {
					...
					for ( var j = 0; j < UpList.length; j++) {
						...
						if (j == 0)
							k = 0;
						if (j == UpList.length - 1)
							k = 2;
						addPointPoup(UpList[j].Longitude, UpList[j].Latitude,
								UpList[j].StationId, list.Line.Name, lineId,
								direct, k);
					}
				}
				if (direct == 2) {
					...
					for ( var j = 0; j < DownList.length; j++) {
						...
						if (j == 0)
							k = 0;
						if (j == DownList.length - 1)
							k = 2;
						addPointPoup(DownList[j].Longitude,
								DownList[j].Latitude, DownList[j].StationId,
								list.Line.Name, lineId, direct, k);
					}
				}
	...
	$$.ajax( {
		...
		success : function(data) {
			...
			if (list != undefined) {
				...
				map.drawLine(coordinates, DRAWLINE_COLOR, "solid",
						DRAWLINE_SIZE);
...
function busStationPoup(lineId, stationId, direct) {
	...
	$$
			.ajax( {
				...
				var html = '<div class="map_bus_div"><div class="map_bus_div1"><label class="map_bus_title">'
						+ StationInfo.name
						+ '</label><label class="map_bus_link" onclick=details("'
						+ lineId
						+ '","'
						+ stationId
						+ '",1)>详情&raquo;</label>'
						+ '<img  class="addTr_div_1_colloct" src="WebPage/image/common/collect.png" onclick=SaveStationFavor(this,"'
						+ lineId + '",1) /></div><div class="map_bus_div2">';
				for ( var j = 0; j < StationLineInfo.length; j++) {
					if (StationLineInfo[j].id == lineId) {
						html += '<a  href="javascript:void(0)" onclick=busStationPoup("'
								+ StationLineInfo[j].id
								+ '","'
								+ stationId
								+ '","'
								+ direct
								+ '") '
								+ ' class="map_bus_div2_select">'
								+ StationLineInfo[j].name + '</a>';
					} else {
						html += '<a  href="javascript:void(0)" onclick=busStationPoup("'
								+ StationLineInfo[j].id
								+ '","'
				...
				var div3 = $$('<div class="map_bus_div3"><label class="poupcontent_busRouteName">'
						+ LineInfo.line.name
						+ '</label>'
						+ '<div class="map_bus_div3_l1">'
						+ upStartStationName
						+ '<img src="WebPage/image/common/busStation_srart_end.png"/>'
						+ upEndStationName
						+ '</div><div class="map_bus_div3_l2"><img src="WebPage/image/common/bus_pass.png"/>途中'
						+ '<img src="WebPage/image/common/refresh.png" onclick="QueryOriginBusArrive()" style="cursor: pointer;" />刷新</div></div>');
				var div4 = $$('<div class="map_bus_div4"><label>首末班时间：'
						+ LineInfo.line.lineUpStartTime + '-'
						+ LineInfo.line.lineUpEndTime + '&nbsp;全程：'
						+ LineInfo.line.price + '元</label></div>');
				var div5 = $$('<div class="map_bus_div5"></div>');// 实时到站
				var maxwindth = upListInfo.length * 32;// ul 宽度
				var content = '<div class="map_bus_div6"><a class="map_bus_div6_left" style="visibility:hidden" onclick="BusStationListMove_left(this)">' + '<img src="WebPage/image/common/arrow_left_normal.png"/></a><div class="map_bus_div6_content"><ul style="width:' + maxwindth + 'px">';
				for ( var i = 0; i < upListInfo.length; i++) {
					...
					if (upListInfo[i].stationId == stationId) {
						content += '<li><a href="javascript:void(0)" onclick=details("'
								+ lineId
								+ '","'
								+ upListInfo[i].stationId
								+ '",1) class=BusSelectStation >'
								+ busStationName + '</a></li>';
					} else {
						content += '<li><a href="javascript:void(0)" onclick=details("'
								+ lineId
								+ '","'
								+ upListInfo[i].stationId
								+ '",1)  >' + busStationName + '</a></li>';
					}
					var k = 1;
					if (i == 0)
						k = 0;
					if (i == upListInfo.length - 1)
						k = 2;
					addPointPoup(upListInfo[i].longitude,
							upListInfo[i].latitude, upListInfo[i].stationId,
							LineInfo.line.name, lineId, direct, k);
				}
				if (maxwindth > 500) {
				...
				var searchContent = '<div class="map_bus_div7"><span class="map_bus_div7_span1" onclick="ChangeSpan_Way(this,1)"><img src="WebPage/image/common/start_icon.png"/>到这里去</span><span onclick="ChangeSpan_Way(this,2)" class="map_bus_div7_span"><img src="WebPage/image/common/end_icon.png"/>从这里出发</span><span onclick="ChangeSpan_Way(this,3)" class="map_bus_div7_span"><img src="WebPage/image/common/search_icon.png"/>附近找</span>'
						+ '<div id="map_bus_div7_content1" class="map_bus_div7_content1"><input type="text" class="input_tex6" id="map_bus_div7_cnt1"/><span class="map_bus_div7_button" onclick=transfer_Way(1,"'
						+ lonlat
						+ '","'
						+ StationInfo.name
						+ '")>公交</span><span class="map_bus_div7_button" onclick=transfer_Way(3,"'
						+ lonlat
						+ '","'
						+ StationInfo.name
						+ '")>自驾</span></div>'
						+ '<div id="map_bus_div7_content2" class="map_bus_div7_content"><input type="text" class="input_tex6" id="map_bus_div7_cnt2"/><span class="map_bus_div7_button" onclick=transfer_Way(2,"'
						+ lonlat
						+ '","'
						+ StationInfo.name
						+ '")>公交</span><span class="map_bus_div7_button" onclick=transfer_Way(4,"'
						+ lonlat
						+ '","'
						+ StationInfo.name
						+ '")>自驾</span></div>'
			...
			if (direct == 2) {
				...
				var html = '<div class="map_bus_div"><div class="map_bus_div1"><label class="map_bus_title">'
						...
						+ '",2)>详情&raquo;</label>'
						+ '<img  class="addTr_div_1_colloct" src="WebPage/image/common/collect.png" onclick=SaveStationFavor(this,"'
						+ lineId + '",2) /></div><div class="map_bus_div2">';
				for ( var j = 0; j < StationLineInfo.length; j++) {
					if (StationLineInfo[j].id == lineId) {
						html += '<a  href="javascript:void(0)" onclick=busStationPoup("'
								+ StationLineInfo[j].id
				...
				var div3 = $$('<div class="map_bus_div3"><label class="poupcontent_busRouteName">'
						...
						+ '</label>'
						+ '<div class="map_bus_div3_l1">'
						+ downStartStationName
						+ '<img src="WebPage/image/common/busStation_srart_end.png"/>'
						+ downEndStationName
						+ '</div><div class="map_bus_div3_l2"><img src="WebPage/image/common/bus_pass.png"/>途中'
						+ '<img src="WebPage/image/common/refresh.png" onclick="QueryOriginBusArrive()" style="cursor: pointer;" />刷新</div></div>');
				var div4 = $$('<div class="map_bus_div4"><label>首末班时间：'
						+ LineInfo.line.lineDownStartTime + '-'
						+ LineInfo.line.lineDownEndTime + '&nbsp;全程：'
						+ LineInfo.line.price + '元</label></div>');
				var div5 = $$('<div class="map_bus_div5"></div>');// 实时到站
				...
				for ( var i = 0; i < downListInfo.length; i++) {
					...
					if (downListInfo[i].stationId == stationId) {
						content += '<li><a href="javascript:void(0)" onclick=details("'
								+ lineId
								+ '","'
								+ downListInfo[i].stationId
								+ '",2) class=BusSelectStation >'
								+ busStationName + '</a></li>';
					} else {
						content += '<li><a href="javascript:void(0)" onclick=details("'
								+ lineId
								+ '","'
								+ downListInfo[i].stationId
								+ '",2)  >' + busStationName + '</a></li>';
					}

					var k = 1;
					if (i == 0)
						k = 0;
					if (i == downListInfo.length - 1)
						k = 2;
					addPointPoup(downListInfo[i].longitude,
							downListInfo[i].latitude,
							downListInfo[i].stationId, LineInfo.line.name,
							lineId, direct, k);
				}
				if (maxwindth > 500) {
					content += '</ul></div><a class="map_bus_div6_right" onclick="BusStationListMove_Right(this)"><img src="WebPage/image/common/arrow_right_normal.png"/></a></div>';
								...
								success : function(data) {
									if (direct == 1) {
										...
										var div4 = $$('<div class="map_bus_div4"><label>首末班时间：'
												+ LineInfo.line.lineUpStartTime
												+ '-'
												+ LineInfo.line.lineUpEndTime
												+ '&nbsp;全程：'
												+ LineInfo.line.price
												+ '元</label></div>');
										var div5 = $$('<div class="map_bus_div5"></div>');// 实时到站
										var maxwindth = upListInfo.length * 32;// ul
																				// 宽度
										var content = '<div class="map_bus_div6"><a class="map_bus_div6_left" style="visibility:hidden" onclick="BusStationListMove_left(this)">' + '<img src="WebPage/image/common/arrow_left_normal.png"/></a><div class="map_bus_div6_content"><ul style="width:' + maxwindth + 'px">';
										for ( var i = 0; i < upListInfo.length; i++) {
											var busStationName = upListInfo[i].stationName;
										...
										var searchContent = '<div class="map_bus_div7"><span class="map_bus_div7_span1" onclick="ChangeSpan_Way(this,1)"><img src="WebPage/image/common/start_icon.png"/>到这里去</span><span onclick="ChangeSpan_Way(this,2)" class="map_bus_div7_span"><img src="WebPage/image/common/end_icon.png"/>从这里出发</span><span onclick="ChangeSpan_Way(this,3)" class="map_bus_div7_span"><img src="WebPage/image/common/search_icon.png"/>附近找</span>'
												...
												+ '")>自驾</span></div>'
												+ '<div id="map_bus_div7_content3" class="map_bus_div7_content"><input type="text" class="input_tex6" id="map_bus_div7_cnt3"/><span class="map_bus_div7_button" onclick="searchKey_Way()">搜索</span></div></div>';
										var popup = map.createPopup(lonlat,
												500, 480, html);
										$$('#poupcontent').empty().append(div3);
										$$('#poupcontent').append(div4);
										$$('#poupcontent').append(div5);
									...
									if (direct == 2) {
										...
										var div4 = $$('<div class="map_bus_div4"><label>首末班时间：'
												+ LineInfo.line.lineDownStartTime
												+ '-'
												+ LineInfo.line.lineDownEndTime
												+ '&nbsp;全程：'
												+ LineInfo.line.price
												+ '元</label></div>');
										var div5 = $$('<div class="map_bus_div5"></div>');// 实时到站
										var maxwindth = downListInfo.length * 32;// ul
																					// 宽度
										var content = '<div class="map_bus_div6"><a class="map_bus_div6_left" style="visibility:hidden" onclick="BusStationListMove_left(this)">' + '<img src="WebPage/image/common/arrow_left_normal.png"/></a><div class="map_bus_div6_content"><ul style="width:' + maxwindth + 'px">';
										for ( var i = 0; i < downListInfo.length; i++) {
											var busStationName = downListInfo[i].stationName;
...
function details(routeId, busStationId, direct) {
	window.open($$("#basePath").val()
			+ "WebPage/page/publicTraffic/wisdom/bus_infor.jsp?routeId="
			+ routeId + "&busStationId=" + busStationId + "&direct=" + direct);
}

// 站点详情页面
function Line_details(routeId, busStationId, direct) {
	window.open($$("#basePath").val()
			+ "WebPage/page/publicTraffic/wisdom/line_infor.jsp?routeId="
			+ routeId + "&busStationId=" + busStationId + "&direct=" + direct);
}
					...
					if (data != "") {
						...
						if (list != "") {
							for ( var i = 0; i < list.length; i++) {
								var linum = Number(Gloval_select_stationlength)
										- Number(list[i].StationIndex);
								$$("#poupcontent #busicon" + linum)
										.html(
												"<img src='WebPage/image/common/bus_pass.png'/>");
							}
						} else {
