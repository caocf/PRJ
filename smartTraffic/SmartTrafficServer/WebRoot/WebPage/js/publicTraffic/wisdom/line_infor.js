...
function PageContent(routeId) {
	...
	$$
			.ajax( {
				...
				var html = '<div class="map_bus_div"><div class="map_bus_div1"><label class="map_bus_title">'
						+ StationName
						+ '</label>'
						+ '<img  class="map_bus_colloct" src="WebPage/image/common/collect.png" onclick=SaveStationFavor(this,"'
						+ requestBusStationId
						+ '","'
						+ StationName
						+ '","'
						...
						+ '","'
						+ lat
						+ '") /><div class="map_bus_save">收藏</div></div><div class="map_bus_div2"><div class="map_bus_road">途径公交</div>';
				GetBusRouteMap(requestBusStationId, busRouteName);
				html += '</div>';
				html += '<div class="map_bus_div3"><label class="poupcontent_busRouteName">'
						+ busRouteName
						+ '</label>'
						+ '<div class="map_bus_div3_l1">'
						+ firstStation
						+ '<img src="WebPage/image/common/busStation_srart_end.png" onclick=ChangeDirect()></img>'
						+ lastStation
						+ '</div>'
						+ '<div class="map_bus_div3_l2"><img src="WebPage/image/common/bus_pass.png" style="margin-right: 5px;vertical-align: middle;">途中'
						+ '<img  src="WebPage/image/common/refresh.png" onclick=QueryOriginBusArrive("'
						+ routeId
						+ '","'
						+ UpList[UpList.length - 1].StationId
						+ '","'
						+ UpList.length
						+ '") style="margin-left: 10px;vertical-align: middle;cursor: pointer;"/></div></div>';
				html += '<div class="map_bus_div4"><label>首末班时间：'
						+ Line.LineUpStartTime + '-' + Line.LineUpEndTime
						+ '&nbsp;全程：' + Line.Price + '元</label></div>';
				...
				QueryOriginBusArrive(routeId,
						UpList[UpList.length - 1].StationId, UpList.length);
			}
			if (direct == 2) {
				var list = JSON.parse(data);
				var Line = list.Line;
				...
				html += '<div class="map_bus_div3"><label class="poupcontent_busRouteName">'
						...
						+ routeId
						+ '","'
						+ DownList[DownList.length - 1].StationId
						+ '","'
						+ DownList.length
						+ '") style="margin-left: 10px;vertical-align: middle;cursor: pointer;"/></div></div>';
				html += '<div class="map_bus_div4"><label>首末班时间：'
						+ Line.LineDownStartTime + '-' + Line.LineDownEndTime
						+ '&nbsp;全程：' + Line.Price + '元</label></div>';
				html += '<div class="map_bus_div5"></div>';
	...
	CreatsCoordinateBylineId(routid, direct);
}
// 站点详情页面
				...
				success : function(data) {
					...
					for ( var i = 0; i < LineList.length; i++) {
						if (busRouteName == LineList[i].Name) {
							html2 += '<a href="javascript:void(0)"  class="map_bus_div2_select" onclick=PageContent("'
									+ LineList[i].Id
									+ '") >'
									+ LineList[i].Name + '</a>';

						} else {
							html2 += '<a href="javascript:void(0)"  class="map_bus_div2_noselect" onclick=PageContent("'
									+ LineList[i].Id
									+ '") >'
									+ LineList[i].Name + '</a>';
						}
...
function creatBusStation(lon, lat) {
	...
	var icon = map.createIcon("WebPage/image/graphical/map_point_pressed.png",
			30);
					...
					if (data != "") {
						...
						if (list != "") {
							for ( var i = 0; i < list.length; i++) {
								var linum = Number(stationlength)
