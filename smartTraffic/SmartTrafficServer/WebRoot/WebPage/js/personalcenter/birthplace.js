...
function initComplexArea(a, k, h, p, q, d, b, l) {//alert("b:"+b+","+"l:"+l);
    ...
    if (p != undefined) {
       ...
       // n[0] = new Option("请选择 ", 0);
        ...
        for (var i = b * 100; i < area.length; i++) {
            if (area[i] == undefined) continue; 
           if(i==l){ str += "<option value='" + i + "' selected='selected' >" + area[i] + "</option>";}
           else{ str += "<option value='" + i + "' >" + area[i] + "</option>";}
        }
        $$("#" + h).html(str);
        }else{
        	n[0] = new Option("请选择 ", 0);
