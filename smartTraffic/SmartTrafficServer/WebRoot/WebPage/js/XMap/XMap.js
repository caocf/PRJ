//相关图标
var aMarker = [ "images/bus/start.png", "images/bus/map_walk.png",
		"images/bus/map_bus.png", "images/bus/map_metro.png",
		"images/bus/end.png" ];
// 定义当前图层组
var currentGroup;
var map; // 定义全局的map
...
function loadTdt() {
	...
	// 比例尺
	// Cfg.mapWMTSLayer_Scales =
	// "2.958293554545656E8,1.479146777272828E8,7.39573388636414E7,3.69786694318207E7,1.848933471591035E7,9244667.357955175,4622333.678977588,2311166.839488794,1155583.419744397,577791.7098721985,288895.85493609926,144447.92746804963,72223.96373402482,36111.98186701241,18055.990933506204,9027.995466753102,4513.997733376551,2256.998866688275,1128.499433441375,564.2497167206875";
	Cfg.mapWMTSLayer_Scales = "2.958293554545656E8,1.479146777272828E8,7.39573388636414E7,3.69786694318207E7,1.848933471591035E7,9244667.357955175,4622333.678977588,2311166.839488794,1155583.419744397,577791.7098721985,288895.85493609926,144447.92746804963,72223.96373402482,36111.98186701241,17042.702728678414,8521.351364339187,4260.675682169598,2254.46776253383,1127.23388126692,563.616940633458";
	// 比例尺转分辨率
	...
	/** ************************嘉兴市18-20级矢量底图**http://220.191.220.71/JXEMAP/wmts************************************** */
	...
	/** ************************嘉兴市18-20级矢量标注底图**http://220.191.220.71/JXEMAPAnno/wmts************************************** */
	...
	/** ************************浙江省15-17级矢量底图**http://srv.zjditu.cn:80/ZJEMAP_2D/wmts************************************** */
	...
	/** ************************浙江省15-17级矢量标注底图**http://srv.zjditu.cn:80/ZJEMAPANNO_2D/wmts************************************** */
	...
	/** ************************全国1-14级矢量底图**http://t0.tianditu.com/vec_c/wmts************************************** */
	...
	/** ************************全国1-14级矢量标注底图**http://t0.tianditu.com/cva_c/wmts************************************** */
	...
	/** ************************全国1-14级影像底图**http://t0.tianditu.com/img_c/wmts************************************** */
	// 1-14级全国影像图层
	...
	/** ************************全国1-14级影像标注底图**http://t0.tianditu.com/cia_c/wmts************************************** */
	// 1-14级全国影像标注图层
	...
	/** ************************全国15-17级影像底图**************************************** */
	// 15-17级全国影像图层
	var wmtsImgLayer_Res_mid = getRes(Cfg.mapWMTSLayer_Scales);
	// 创建影像15-17级底图图层
	var wmtsImageLayer_mid = new Geo.View2D.Layer.WMTS({
		name : "全国15-17级影像底图",
		...
		layer : "ZJDOM_2D",
		zoomOffset : 1,
		resolutions : wmtsImgLayer_Res_mid,
		maxResolution : wmtsLayer_Res[14],// 最大分辨率
		minResolution : wmtsLayer_Res[16],// 最小分辨率
		format : "tiles",
		tileFullExtent : Geo.Bounds.fromString("-180.0,-90.0,180.0,90.0")
	...
	/** ************************全国15-17级影像标注底图**************************************** */
	// 15-17级全国影像标注图层
	var wmtsAnnoImgLayer_Res_mid = getRes(Cfg.mapWMTSLayer_Scales);
	// 创建影像15-17级标注图层
	var wmtsAnnoImageLayer_mid = new Geo.View2D.Layer.WMTS({
		name : "全国15-17级影像标注底图",
		...
		layer : "ZJDOMANNO_2D",
		zoomOffset : 1,
		resolutions : wmtsAnnoImgLayer_Res_mid,
		maxResolution : wmtsLayer_Res[14],// 最大分辨率
		minResolution : wmtsLayer_Res[16],// 最小分辨率
		format : "tiles",
		tileFullExtent : Geo.Bounds.fromString("-180.0,-90.0,180.0,90.0")
	...
	/** ************************嘉兴市18-20级影像底图**http://220.191.220.71/JXIMG/wmts************************************** */
	...
	/** ************************嘉兴市18-20级影像标注底图**http://220.191.220.71/JXIMGAnno/wmts************************************** */
	...
	imageGroup = new Geo.View2D.LayerGroup({
		layers : [ wmtsImageLayer_qg, wmtsImageLayer_mid, wmtsImageLayer_jx,
				wmtsAnnoImageLayer_qg, wmtsAnnoImageLayer_mid,
				wmtsAnnoImageLayer_jx ]
	});
	// 定义基础图层组
...
function XMap(mapDiv, type, json) {
	...
	if (type == 1) {
					...
					function() {
						...
						if (image.indexOf("weixingditub", 0) != -1) {
							...
							$$("#nav").css("background-image",
									"url(images/dianziditub.png)");
						} else {
							switchGroup(baseLayerGroup);
							$$("#nav").css("background-image",
									"url(images/weixingditub.png)");
						}
					});
		}
		/**
		 ...
		 * @param tagMarker
		 */
		map.removeTagMarker = function removeTagMarker(tagMarker) {
			markersLayer.removeMarker(tagMarker);
		}
		/**
		 * 删除所有tagMarkers
		 */
		map.removeTagMarkers = function removeTagMarkers() {
		 ...
		 * @returns {Geo.LonLat}
		 */
		map.createLonlat = function createLonlat(lonlat1) {

			var lonlat = new Geo.LonLat(lonlat1.split(',')[0], lonlat1.split(',')[1]);
			return lonlat;
		}
		/**
		 * 创建图标对象
		 ...
		 */
		map.createPopupObject = function createPopupObject(id, lonlat,
				contentSize, contentHTML, anchor, closeBox) {
			var popup = new Geo.View2D.Popup.FramedCloud(id, lonlat,
					contentSize, contentHTML, anchor, closeBox);
			return popup;
		}
		/**
		 ...
		 * 标签标注只适应屏幕
		 */
		map.createTagMarkerSuitable = function createTagMarkerSuitable() {
			var bound = markersLayer.getDataExtent();
			map.setMapExtent(bound, false);
		}
		/**
		 ...
		 *            泡泡窗口高度
		 */
		map.addPopupForFeature = function addPopupForFeature(lonlat, imageIcon,
				label, size, color, font, html, htmlwidth, htmlheight) {
			polygonFeature = map.addCustomerMarker(lonlat, imageIcon, label,
					size, color, font);
			// 添加要素到图层中
		 ...
		 */
		map.addCustomerMarker = function addCustomerMarker(lonlat, image,
				label, imageSize, fontColor, fontFamily) {
		...
		map.addTagMarker = function addTagMarker(lonlat, image, html, imageSize) {
			lonlat = map.createLonlat(lonlat);
			var tag = new Geo.View2D.MarkerTag(html);
			tag.offset = new Geo.Pixel(-(tag.size.w / 2), -(tag.size.h / 1.5));
			var size = new Geo.Size(imageSize, imageSize);
			var icon = new Geo.View2D.Icon(image, size);
			geoMarker = new Geo.GeoMarker(lonlat, icon, tag)
		 ...
		 *            浮云内容
		 */
		map.createPopup = function createPopup(lonlat, width, height,
				contentHTML) {
			var lonlat = map.createLonlat(lonlat);
			var size = map.createPopupSize(width, height);
			currentPopup = map.createPopupObject(null, lonlat, size,
		 ...
		 *            浮云高度
		 */
		map.createPopupForMarker = function createPopupForMarker(lonlat,
				content, imageIcon, imageSize, width, height) {
			var lonlat = map.createLonlat(lonlat);
			var size = map.createPopupSize(width, height);
			currentPopup = map.createPopupObject(null, lonlat, size, content,
					null, true);
			var feature = new Geo.Feature(markers, lonlat);
			feature.closeBox = true;
			feature.popupClass = AutoSizeFramedCloud;
		 ...
		 * @returns {Geo.Feature.Vector}
		 */
		map.drawLine = function drawLine(lineString, color, dashstyle,
				strokeWidth) {
			...
			if (lonlatslength > 0) {
				...
				// 定位视野范围
				// map.zoomToExtent(new Geo.Bounds(ymin, xmin, ymax, xmax),
				// false);
		...
		/*
		 * map.drawLine = function drawLine(lineString, color,
		 * dashstyle,strokeWidth) { if(typeof(strokeWidth)=='undefined'){
		 * strokeWidth=3; } var lineFeature; var line = lineString; var lonlats =
		 * line.split(";"); var lonlatslength = lonlats.length; if
		 * (lonlatslength > 0) { var lineArr = new Array(); for ( var i = 0; i <
		 * lonlatslength; i++) { var lonlat = lonlats[i]; if (lonlat == "") {
		 * continue; } var lon = lonlat.split(",")[0]; var lat =
		 * lonlat.split(",")[1]; lineArr.push(new Geo.Geometry.Point(lon, lat));
		 * setCenter(lon + "," + lat, 14); } var line1 = new
		 * Geo.Geometry.LineString(lineArr); var style1 = { strokeWidth :
		 * strokeWidth, strokeOpacity : 1, strokeColor : color, strokeDashstyle :
		 * dashstyle }; lineFeature = new Geo.Feature.Vector(line1, null,
		 * style1); vectorLayer.addFeatures(lineFeature); } return lineFeature; }
		 */
		/**
		 * 画圆，针对周边查询的圆查询
		 * 
		...
		map.drawRadio = function drawRadio(loc, m, color, dashstyle) {
			var lonlat = new Geo.Geometry.Point(loc.split(",")[0], loc
					.split(",")[1]);
			var polygon = new Geo.Geometry.Polygon.createRegularPolygon(lonlat,
					m, 40, 360);
			style = {
				strokeWidth : 2,
				strokeOpacity : 1,
		...
		map.addRect = function addRect(number, color) {
			...
			var rect = new Geo.View2D.Control.DrawFeature(polygonStyleLayer,
					Geo.View2D.Handler.RegularPolygon, {
						handlerOptions : {
							sides : number,
							irregular : true
		...
		map.clickToAddMarker = function clickToAddMarker(imageIcon) {
			...
			Geo.View2D.Control.Click = Geo.Class(Geo.View2D.Control, {
				...
				onClick : function(evt) {
					...
					var lonlat = new Geo.LonLat(lonlat.split(",")[0], lonlat
							.split(",")[1]);
					var icon = new Geo.View2D.Icon(imageIcon, new Geo.Size(30,
							30));
					var marker = new Geo.Marker(lonlat, icon);
					markers.addMarker(marker);
					map.addLayer(markers);
	...
	if (type == 2) {
		...
		wmap.getWGS84ByBound = function getWGS84ByBound(lon, lat) {
			var lonlat = WebtAPI.WUtil._toWGS84(wmap.createLonlat(lon + ","
					+ lat));
			return lonlat;
		}
		// 创建图标对象
		wmap.createIcon = function createIcon(imageIcon, size1) {
			...
			var icon = new WebtAPI.WIcon(imageIcon, new OpenLayers.Size(size1,
					size1), null, calculateOffset);
			return icon;
		}
		// 创建图标对象
		...
		// 创建关键字查询对象
		wmap.createKeyQuery = function createKeyQuery(key, pageCount,
				pageNumber) {
		...
		// 创建分类查询对象
		wmap.createTypeQuery = function createTypeQuery(type, pageCount,
				pageNumber) {
		...
		// 创建周边查询对象
		wmap.createSurroundQuery = function createSurroundQuery(lon, lat, key,
				radius, pageCount, pageNumber) {
		...
		// 创建驾车查询对象
		wmap.createRouteQuery = function createRouteQuery(point, costModel,
				avoidArea) {
			...
			routeQuery.setOption("criteria", 0);
		...
		// 返回数组，为经纬度的128秒
		wmap.createLonlat2Second128Lonlat = function createLonlat2Second128Lonlat(
				lonlat) {
			return [ Math.round(lonlat.lon * 128 * 3600),
					Math.round(lonlat.lat * 128 * 3600) ];
		}
		/** ******************创建对象************************ */
		// 获取84经纬度
		wmap.getWGS84 = function getWGS84(position) {
			var lonlat = WebtAPI.WUtil._toWGS84(map
					.getLonLatFromViewPortPx(position));
			return lonlat;
		}
		// 添加右键功能
		...
		// rightToGetLonlat();
		/*
		 * //是鼠标右键生效 wmap.addContextMenu=function addContextMenu(){
		 * rightMenu.activate(); } //关闭鼠标右键 wmap.closeContextMenu=function
		 * closeContextMenu(){ rightMenu.deactivate(); }
		 */
		// 添加比例尺
		...
		wmap.addNavType = function addNavType() {
			...
			var layer = [
					wmap.chiDayLayer = new WebtAPI.TileCacheLayer(
							"chiDayLayer",
							"http://192.168.10.192/fmapimg_chi/", {
								isBaseLayer : false,
								visibility : false
							}),
					wmap.engDayLayer = new WebtAPI.TileCacheLayer(
							"engDayLayer",
							"http://192.168.10.192/fmapimg_eng/", {
								isBaseLayer : false,
								visibility : false
							}),
					wmap.chiNightLayer = new WebtAPI.TileCacheLayer(
							"chiNightLayer",
							"http://192.168.10.192/fmapimg_chi_n/", {
								isBaseLayer : false,
								visibility : false
							}),
					wmap.engNightLayer = new WebtAPI.TileCacheLayer(
							"engNightLayer",
							"http://192.168.10.192/fmapimg_eng_n/", {
								isBaseLayer : false,
								visibility : false
							}) ];
		...
		wmap.drawLine = function drawLine(lineString, color, dash, strokeWidth) {
			...
			if (line.indexOf(";") != -1) {
				...
				if (lonlatslength > 0) {
					...
					var lineFeature = new WebtAPI.LineFeature(lineArr,
							lineStyle);
		...
		// -------------------------过时方法
		wmap.removeAll = function removeAll() {
			wmap.dynamicVectorLayer.removeAllFeatures();
		}
		// 删除所有marker-----------------过时方法
		...
		wmap.drawMarker = function drawMarker(lonlat, imageIcon, size) {
			var lonlat = new OpenLayers.LonLat(lonlat.split(",")[0], lonlat
					.split(",")[1]);
		...
		wmap.addMarker = function addMarker(lonlat, imageIcon, size) {
			...
			var icon = new WebtAPI.WIcon(imageIcon,
					new OpenLayers.Size(30, 30), null, calculateOffset);
			var marker = new WebtAPI.WMarker(lonlat, icon);
			wmap.markersLayer.addMarker(marker);
			return marker;
		}
		...
		// 给marker添加popup
		wmap.createPopupForMarker = function createPopupForMarker(lonlat,
				content, imageIcon, imageSize, width) {
			...
			marker.register("click",
					function() {
						var html = content;
						var popup = new WebtAPI.WPopup(null, marker, width,
								html, true);
						popup.setToolbarDisplay(false);
						// 添加POPUP到地图中。
						wmap.addPopup(popup, true);
		...
		wmap.drawRadio = function drawRadio(loc, m, color, dash) {
			...
			var ringFeature = OpenLayers.Geometry.Polygon.createRegularPolygon(
					new OpenLayers.Geometry.Point(aroLL.lon, aroLL.lat), m, 40,
					360);
			var lineStyle = WebtAPI.style["line"];
		...
		wmap.addRect = function addRect(number) {
			polygonControl = new OpenLayers.Control.DrawFeature(
					wmap.dynamicVectorLayer, OpenLayers.Handler.RegularPolygon,
					{
						handlerOptions : {
							sides : number,
							irregular : true
		...
		wmap.drawPolygon = function drawPolygon(lineString, color) {
			...
			var feature = new WebtAPI.PolygonFeature(points, style, {
				code : "110000",
				id : "xxx"
			});

			control = new WebtAPI.WControl.PolygonPainter({
				callback : {
					"rightclick" : rightCallback
				}
			});
	...
	if (type == 3) {
		...
		maplet.centerAndZoom(
				new MPoint(120.75121946334839, 30.747786235809325), 8);
		/**
		 * 创建经纬度对象
		 * 
		 * @param lonlat
		...
		maplet.createLonlat = function createLonlat(lonlat) {
			var object = new MPoint(lonlat.split(",")[0] + ","
					+ lonlat.split(",")[1]);
			return object;
		}
		/**
		 ...
		 *            线条宽度
		 */
		maplet.drawLine = function drawLine(lineString, color, dashstyle,
				strokeWidth) {
			if (dashstyle == "solid") {
				dashstyle == 0;
			}
			...
			if (lonlatslength > 0) {
				...
				polyline = new MPolyline(lineArr, brush, new MInfoWindow(
						"中山东路", "全长---"));
		 ...
		 * 
		 * @param polyline
		 *            折线对象-----------该方法过时，建议使用removeFeature(feature)
		 */
		 ...
		 * @returns {___brush1}
		 */
		maplet.createBrush = function createBrush(color, dash, stroke, bgcolor,
				isfill) {
			var brush = new MBrush(color);
			brush.style = dash;
			brush.stroke = stroke;
		 ...
		 * @param height
		 */
		maplet.createPopupForMarker = function createPopupForMarker(lonlat,
				content, imageIcon, imageSize, width, height) {
			var lonlat = maplet.createLonlat(lonlat);
		...
		maplet.addNavType = function addNavType() {
			maplet.showSatelliteMap(false);
		}
	}
...
function onFeatureSelect(feature) {
	...
	currentPopup = new Geo.View2D.Popup.FramedCloud("chicken", feature.geometry
			.getBounds().getCenterLonLat(), feature.data.popupSize,
			feature.data.popupContentHTML, null, true, onPopupClose);
