...
public class ParkingDBAction {
	...
	public void setParkTypes(String parkTypes) {
		this.parkTypes = parkTypes;
	}

	public String searchParkingByNameAndTypes() {

		try {
			int[] types = null;
			if (parkTypes != null && !parkTypes.equals("")) {
				String[] strs = parkTypes.split(",");

				types = new int[strs.length];

				for (int i = 0; i < strs.length; i++) {
					types[i] = Integer.valueOf(strs[i]);
				}
			}
			parkName = parkName == null ? "" : parkName;

			result = this.parkingDBService.searchParkingByNameAndTypes(types,
					parkName, page, num);
	...
	public String searchParkingByLocationCircle() {
		try {
			result = this.parkingDBService.searchParkingByLocationCircle(
					radius, lan, lon, page, num, isReal == 1 ? true : false);
