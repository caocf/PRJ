...
//只能输数字和逗号
function isNumberAndDouhao(valueId, errorId, msg) {
	var str = $.trim($(valueId).val());
	var reg = /^[0-9,]+$/;
	if (str == "") {
		rightStye(valueId, errorId);
...
//整数和小数
function CheckNumberOrDouble(valueId, errorId, msg) {
	var str = $(valueId).val();
	if (str == "") {
		rightStye(valueId, errorId);
		return true;
	...
	if (reg.test(str)||reg2.test(str)) {
		...
		errorStye(valueId, errorId, msg + "格式不正确，整数或小数");
		return false;
	}
}
...
function arithmetic(value,valueTo,errorTitile){
	try{
		$(valueTo).attr("value",RoundNumber(eval(value),0));
		return true;
		}catch(exception) {
			$(valueTo).attr("value","");
 ...
 * @param {Object} value
 */
function checkIP(valueId, errorId, msg) {
	var str = $(valueId).val();
