...
function HS_calender() {
	...
	style += ".calenderTime{clear:both;}";
	style += ".calenderTime select{width:auto;height:auto}";
	style += ".calenderBottom {clear:both; border-top:1px solid #ddd; padding: 3px 0; text-align:right;height:20px;line-height:20px}";
	style += ".calenderBottom a {text-decoration:none; margin:2px !important; font-weight:bold; color:#000}";
	style += ".calenderBottom a:hover{color: #4995F5;}";
	...
	for (i = 1; i <= thisMonthEndDate; i++) { // Current Month's Date
		if (today == now.getFullYear() + "-" + now.getMonth() + "-" + i) {
			...
			lis += "<li><a href=javascript:void(0) class='today' onclick='_selectThisDay(this)' title='"
					...
					+ "-" + i + "'>" + i + "</a></li>";
			selecttimebycalender=now.getFullYear()+ "-"+ (parseInt(now.getMonth()) + 1)+ "-" + i;
		} else {
	...
	if (arguments.length > 1) {
		...
		var CalenderBox = style
				+ "<div id='Date' class='calender'><div class='calenderTitle'>"
				+ CalenderTitle
				...
				+ lis
				+ "</ul></div>";
		CalenderBox+="<div class='calenderTime'><select id='calenderHour' class='calender_select'>";
		for (var n=0; n < 24; n++) {
			CalenderBox+="<option value='"+n+"'>"+n+"</option>";
		}
		CalenderBox+="</select>时<select id='calenderMin' class='calender_select'>";
		for (var n=0; n < 60; n++) {
			CalenderBox+="<option value='"+n+"'>"+n+"</option>";
		}
		CalenderBox+="</select>分<select id='calenderSed' class='calender_select'>";
		for (var n=0; n < 60; n++) {
			CalenderBox+="<option value='"+n+"'>"+n+"</option>";
		}
		CalenderBox+="</select>秒</div>";
		CalenderBox+="<div class='calenderBottom'><a href=javascript:void(0)  onclick='_AddVlaueByselectThisDay(this)'>确定</a>" +
				"<a href=javascript:void(0)  onclick='_clearCalender(this)' >清空</a>" +
				"<a href='javascript:void(0)' class='closeCalender' onclick='closeCalender(this)'>取消</a></span></div></div>";
		return CalenderBox;
	}
}
function _selectThisDay(d) {
	var boxObj = d.parentNode.parentNode.parentNode.parentNode.parentNode;
	selecttimebycalender= d.title;
	$(".calender .today").attr("class","");
	d.className="today";
	//boxObj.parentNode.removeChild(boxObj);
}
function _AddVlaueByselectThisDay(d){
	var boxObj = d.parentNode.parentNode.parentNode;
	var timevalue=selecttimebycalender+" "+document.getElementById("calenderHour").value+":"+document.getElementById("calenderMin").value+":"+document.getElementById("calenderSed").value;
	boxObj.targetObj.value = formatDate(timevalue,"yyyy-MM-dd HH:mm:ss");
	boxObj.parentNode.removeChild(boxObj);
}
function _clearCalender(d){
	var boxObj = d.parentNode.parentNode.parentNode;
...
function HS_setDate(inputObj,margin_top,margin_left) {
	var calenderObj = document.createElement("span");
	calenderObj.innerHTML = HS_calender(new Date());
	calenderObj.style.position = "absolute";
	calenderObj.targetObj = inputObj;
	//calenderObj.style.left=margin_left+"px";
	//calenderObj.style.top=margin_top+"px";
	//calenderObj.style.zIndex="20";
 ...
 * $(document).click(function(event){
	if( $(event.target).attr("class") != "calender"&&$(event.target).attr("class") != "input_time"&&$(event.target).attr("class")!="calender_select" ){
		$(".calender").hide();
	}
});
