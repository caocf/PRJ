...
function gotobyajax(pinfo,dgId, pageSize, currPage, params, fromMVId){
		//console.log("10");
	var url = pinfo.resPath + "control/" + pinfo.extAction+".servlet?"+pinfo.sidKey+"=ext.sys.fenye.ajax&currPage="+currPage+"&id="+dgId+"&pageSize="+pageSize+"&"+pinfo.fromId+"="+fromMVId;
	__showLoading();
	jQuery.ajax({
	   type: "POST",
	   url: url,
...
function extColOrder(dgId, order, fromMVId, params){
	...
	var url = extAction+".servlet?"+sidKey+"=ext.sys.fenye.ajax&ext_col_order="+order+"&id="+dgId+"&ext_order_state="+jQuery('#ext_order_state').val()+"&"+fromId+"="+fromMVId;
...
function post2Comp(targetId, url, ff, paramNames){
		//console.log("16");
	 url =url.replace("extControl?","extControl.servlet?"); 
                url = url.replace("extView?","extView.servlet?");  
	var parms = '';
...
function post2MV(config){
	...
	for(i=0; i<config.length; i++){
		 var url = config[i].url.replace("extControl?","extControl.servlet?"); 
                url = config[i].url.replace("extView?","extView.servlet?");  
		__post2MV(config[i].target, url, config[i].paramNames, config[i].fname);
	}
}
	...
	obj.live('click', function(e){
		...
		if(a.size() > 0){
			...
			for(i=0; i<config.url.length; i++){
				 var u = config.url[i].url;
                                config.url[i].url=u.replace("extControl?","extControl.servlet?"); //  extView
                                config.url[i].url=u.replace("extView?","extView.servlet?");
                                u = config.url[i].url;
	...
	obj.live('click', function(e){
		...
		for(i=0; i<config.url.length; i++){
			var url = config.url[i].url;
                      config.url[i].url=url.replace("extControl?","extControl.servlet?");  
                      config.url[i].url=url.replace("extView?","extView.servlet?");  
...
//	console.log("25");
	...
	pp.drillDim = x;
	var u = url;
   u=url.replace("extControl?","extControl.servlet?");  
   u=url.replace("extView?","extView.servlet?");  
	var d = compId;
	__showLoading();
...
function chartComp_Link(x, xval, url, pms, compId, tps){
	...
	for(i=0; i<url.length; i++){
		var u = url[i];
                url[i]=u.replace("extControl?","extControl.servlet?"); 
                url[i]=u.replace("extView?","extView.servlet?"); 
...
function fieldDirll(config){
	...
	var fields = jQuery('#' + config.table + " td[drillid='"+config.pid+"'] .crossDirll").bind('click', function(e){
		...
		if(thiz.attr('isopen') == 0){
			var tabTr = thiz.parent().css
			('font-weight', 'bold').parents("tr.tr-row1,tr.tr-row2");
			//获取内容
			__showLoading();
			...
			var url = config.url;
            config.url=url.replace("extControl?","extControl.servlet?");  
            config.url=url.replace("extView?","extView.servlet?");  
...
//	console.log("32");
        config.url=config.url.replace("extControl?","extControl.servlet?"); 
        config.url=config.url.replace("extView?","extView.servlet?"); 
