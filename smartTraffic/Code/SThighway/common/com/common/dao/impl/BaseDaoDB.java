...
public class BaseDaoDB<E> implements BaseDao<E> {
	...
	public BaseQueryRecords findOrderBy(E o, String key, Object value,
		...
		try {
			...
			if (page > 0 && rows > 0) {
				int total = count(o, key, value).intValue();
				criteria.setFirstResult((page - 1) * rows);
				criteria.setMaxResults(rows);
				return new BaseQueryRecords(criteria.list(), total, page, rows);
			} else {
	...
	/**
	 * 使用sql语句查询数据 集，带分页(针对数据量大的时候，不同的计算总数的方式)
	 * 
	 * @param sql
	 *            ： sql语句
	 * @param sqlcount
	 ...
	 * @return: 数据集
	 */
	protected BaseQueryRecords findNewC(SQL sql,String sqlcount, int page, int rows) {
		try {
			Query q = getCurrentSession().createSQLQuery(sql.toString());
			Query qcount = getCurrentSession().createSQLQuery(sqlcount);
			// page和rows 都 >0 时返回分页数据
			if (page > 0 && rows > 0) {
				int total = ((BigInteger)qcount.uniqueResult()).intValue();
				q.setFirstResult((page - 1) * rows);
				q.setMaxResults(rows);
				return new BaseQueryRecords(q.list(), total, page, rows);
			} else {
	...
	@Override
	public int countSameName(String key, String value, String tableName,String bhKey,Object bhValue) {
	    StringBuffer sql = new StringBuffer("select count(*) from "+tableName +" where "+key
	    		+" ='"+value+"' ");
	    if(bhValue!=null){
	    	sql.append(" and "+bhKey+" <> ");
	    	if(bhValue instanceof String){
	    		sql.append("'"+bhValue+"'");
	    	}else{
	    		sql.append(bhValue.toString());
	    	}
	    }
	    try {
			int cnt = 0;
	...
	public String selectnameByBh(String tablename, String selectKey,
		...
		StringBuffer sql = new StringBuffer("select "+selectKey+" from "+tablename+" where "+bhkey+"= ");
		if(bhvalue instanceof String){
    		sql.append("'"+bhvalue+"'");
    	}else{
    		sql.append(bhvalue.toString());
    	}
		return (String) this.findUnique(new SQL(sql.toString()));
	}
