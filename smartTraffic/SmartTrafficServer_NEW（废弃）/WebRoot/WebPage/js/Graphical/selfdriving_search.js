...
function OnLoadPage(){
	if($$("#sc_startPoint").val()!="null"){
		...
		if($$("#sc_startPointLonLat").val()!="null")
			...
			markerS = createMarker($$("#sc_startPointLonLat").val(),"WebPage/image/common/map_point_start.png");
			}
		if($$("#sc_endPointLonLat").val()!="null")
		{
			$$("#endPointLonLat").val($$("#sc_endPointLonLat").val());
			markerE = createMarker($$("#sc_endPointLonLat").val(),"WebPage/image/common/map_point_end.png");
		}
		searchCar(null,'1')
	}
}
...
function SelectPoint(type,name,address,lonlat){
	...
	if(type==1){
		$$("#startPoint").val(name);
		$$("#startPointLonLat").val(lonlat);
		map.addMarker(lonlat,"WebPage/image/common/map_point_start.png",30);
		if($$("#endPointLonLat").val()==""){
			ShowNameList("searchpoibykey",1,"endPoint",2);
		}
	}else if(type==2){
		$$("#endPoint").val(name);
		$$("#endPointLonLat").val(lonlat);
		map.addMarker(lonlat,"WebPage/image/common/map_point_end.png",30);
	}else{
		type=type-3;
