...
function PageContent(routeId) {
	...
	$$.ajax( {
		...
		success : function(data) {
			...
			'<img  class="map_bus_colloct" src="WebPage/image/common/collect.png" onclick=SaveStationFavor(this,"'+requestBusStationId+'","'+StationName+'","'+lon+'","'+lat+'") /><div class="map_bus_save">收藏</div></div><div class="map_bus_div2"><div class="map_bus_road">途径公交</div>';
			...
			html+='<div class="map_bus_div3"><label class="poupcontent_busRouteName">' + busRouteName + '</label>'+
					...
					'<img  src="WebPage/image/common/refresh.png" onclick=QueryOriginBusArrive("'+routeId+'","'+ UpList[UpList.length-1].StationId+'","'+UpList.length+'") style="margin-left: 10px;vertical-align: middle;cursor: pointer;"/></div></div>';
			...
			if(direct==2){
				...
				html+='<div class="map_bus_div3"><label class="poupcontent_busRouteName">' + busRouteName + '</label>'+
						'<div class="map_bus_div3_l1">'+firstStation+'<img src="WebPage/image/common/busStation_srart_end.png"/>'+lastStation+'</div>'+
						'<div class="map_bus_div3_l2"><img src="WebPage/image/common/bus_pass.png" style="margin-right: 5px;vertical-align: middle;">途中'+
						'<img  src="WebPage/image/common/refresh.png" onclick=QueryOriginBusArrive("'+routeId+'","'+ DownList[DownList.length-1].StationId+'","'+DownList.length+'") style="margin-left: 10px;vertical-align: middle;cursor: pointer;"/></div></div>';
				...
				if(data!=""){
					...
					if(list!=""){
						for(var i=0;i<list.length;i++){
							...
							$$("#busicon"+linum).html("<img src='WebPage/image/common/bus_pass.png'/>");
						}
					}else{
