...
function transfer_Way(type,LonLat,name){
	...
	if(type==1){
		if($$("#map_bus_div7_cnt1").val()==""){
			...
			window.open($$("#basePath").val()+"WebPage/page/publicTraffic/wisdom/transfer_search.jsp?startPoint="+
		...
		if($$("#map_bus_div7_cnt1").val()==""){
			...
			window.open($$("#basePath").val()+"WebPage/page/Graphical/selfdriving_search.jsp?startPoint="+
			name+"&endPoint="+$$("#map_bus_div7_cnt1").val()+"&startPointLonLat="+LonLat);
		}
	}else if(type==4){
		if($$("#map_bus_div7_cnt2").val()==""){
...
function searchKey_Way(){
	if($$("#map_bus_div7_cnt3").value==""){
		...
		window.open($$("#basePath").val()+"WebPage/page/Graphical/key_search.jsp?KeyPoint="+$$("#map_bus_div7_cnt3").val());
	}
