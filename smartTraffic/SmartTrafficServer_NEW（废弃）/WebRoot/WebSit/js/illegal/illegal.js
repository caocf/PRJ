var carType=["车辆类型","小型汽车","大型货车","摩托车","外籍车","农用车","警用汽车"];
...
function CheckSearchContent(){
	var ret = true;
	ret = emptyChk("#CarNumber", "#loginerr", "车牌号码不能为空.");
	if (ret == true) {
		ret = valueLength("#CarNumber", "#loginerr", "3", "20", "车牌号码是3-20位");
	}
	if (ret == true) {
		ret = emptyChk("#EngineNumber", "#loginerr", "发动机号不能为空.");
	}
	if (ret == true) {
		ret = valueLength("#EngineNumber", "#loginerr", "3", "20", "发动机号是3-20位");
	}
	if (ret == true) {
		ret = emptyChk("#FrameNumber", "#loginerr", "车架号不能为空.");
	}
	if (ret == true) {
		ret = valueLength("#FrameNumber", "#loginerr", "3", "20", "车架号是3-20位");
	}
	return ret;
}
...
function appendToTable(list, listLength){
	...
	for ( var i = 0; i < listLength; i++) {
		...
		newTr.append($$("<td>"+carType[list[i].carType]+"</td>"));
