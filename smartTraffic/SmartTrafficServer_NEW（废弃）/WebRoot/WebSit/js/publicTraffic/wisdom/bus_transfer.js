...
function queryBusTransfer(){
	//清空页面
	EmptyDate();
	if(startpointlonlat==endpointlonlat&&startpointlonlat!=""){
		alert("起终点不能相同！");
		return;
	}
	$$.ajax( {
...
function TableIsNull(tishi){
	$$(".layout1_left_datalist3").append($$("<div class='layout1_left_datalist_tishi'>"+tishi+"</div>"));
}
...
function appendToTable(list, listLength){
	...
	for ( var i = 0; i < listLength; i++) {
		...
		//起点标志
		newTr.append($$("<div class='addDiv_div_ends'><img src='WebSit/image/graphical/icon_origin.png'/><label>"+startLocation+"</label></div>"));
		...
		//终点标志
		newTr.append($$("<div class='addDiv_div_ends'><img src='WebSit/image/graphical/icon_destenation.png'/><label>"+$$("#endPoint").val()+"</label></div>"));
		newDiv.append(newTr);
...
function busRouteCoordinate(num) {
	...
	for (var k = 0; k < coordinate.length; k++) {
		if(coordinate[k][0]==0){
			...
			addPointPoup(spstr[spstr.length-2],"WebSit/image/common/map_point_bus.png",21);
			addPointPoup(spstr[0],"WebSit/image/common/map_point_bus.png",21);
	...
	addPointPoup(startpointlonlat,"WebSit/image/common/map_point_start.png",36);
	addPointPoup(endpointlonlat,"WebSit/image/common/map_point_end.png",36);
	        ...
	        success:function(data){ 
	            ...
	            if(featureMember!=null){
					...
					for ( var i = 0; i < featureMember.length; i++) {
						...
						map.createPopupForMarker(lonlat,html,"WebSit/image/map_icon/"+(i+1)+".png",36,80,60);
					
					}
					  str+="</table>";
			            $$(".layout1_left_datalist3").html(str);
	            }else{
	            	TableIsNull("没有符合条件的起点");
	            }
...
function CreatKeyPoint(thisop,name,address,lonlat,type){
	...
	/*var LonLat = map.createLonlat(lonlat);
	var icon = map.createIcon("WebSit/image/graphical/map_point_pressed.png",36);
	imgMarker = map.createMarker(LonLat, icon);
	map.addMarkerObject(imgMarker);
