	...
	$$("#searchReasultCount").text(StationList.length);
		...
		if (StationList && StationList.length > 0) {
			...
			for ( var i = 0; i < len; i++) {
				...
				if(i<100){
					str += "<li id='addLi_"+ i +"' onclick=stationPoup(this,'"+obj.Id+"','"+obj.List[0].Id+"','"+obj.List[0].Direct+"') class='addLi'>" +
						"<div class='addLi_img'><img id='img_"+i+"' src='WebSit/image/map_icon/"+(i+1)+".png'/></div>"+
						"<div class='addLi_right'><label class='addLi_name' >"+ obj.Name+"</label></div></li>";
				}else{
					str += "<li id='addLi_"+ i +"' onclick=stationPoup(this,'"+obj.Id+"','"+obj.List[0].Id+"','"+obj.List[0].Direct+"') class='addLi'>" +
						"<div class='addLi_img'><img id='img_"+i+"' src='WebSit/image/graphical/map_point_normal.png'/><label>"+(i+1)+"</label></div>"+
						"<div class='addLi_right'><label class='addLi_name' >"+ obj.Name+"</label></div></li>";
				}
	...
	success : function(data) {
		if(direct==1){
			...
			var div3=$$('<div class="map_bus_div3"><label class="poupcontent_busRouteName">' + LineInfo.line.name + '</label>'+
						'<div class="map_bus_div3_l1">'+upStartStationName+'<img src="WebSit/image/common/busStation_srart_end.png"/>'+
						upEndStationName+'</div><div class="map_bus_div3_l2"><img src="WebSit/image/common/bus_pass.png"/>途中'+
						'<img src="WebSit/image/common/refresh.png" onclick="QueryOriginBusArrive()" style="cursor: pointer;" />刷新</div></div>');
			...
			var content = '<div class="map_bus_div6"><a class="map_bus_div6_left" style="visibility:hidden" onclick="BusStationListMove_left(this)">'+
							'<img src="WebSit/image/common/arrow_left_normal.png"/></a><div class="map_bus_div6_content"><ul style="width:'+maxwindth+'px">';
			...
			var lonlat=lon+","+lat;
			var searchContent='<div class="map_bus_div7"><span class="map_bus_div7_span1" onclick="ChangeSpan_Way(this,1)"><img src="WebSit/image/common/start_icon.png"/>到这里去</span><span onclick="ChangeSpan_Way(this,2)" class="map_bus_div7_span"><img src="WebSit/image/common/end_icon.png"/>从这里出发</span><span onclick="ChangeSpan_Way(this,3)" class="map_bus_div7_span"><img src="WebSit/image/common/search_icon.png"/>附近找</span>'+
		...
		if(direct==2){
			...
			var div3=$$('<div class="map_bus_div3"><label class="poupcontent_busRouteName">' + LineInfo.line.name + '</label>'+
						'<div class="map_bus_div3_l1">'+downStartStationName+'<img src="WebSit/image/common/busStation_srart_end.png"/>'+
						downEndStationName+'</div><div class="map_bus_div3_l2"><img src="WebSit/image/common/bus_pass.png"/>途中'+
						'<img src="WebSit/image/common/refresh.png" onclick="QueryOriginBusArrive()" style="cursor: pointer;" />刷新</div></div>');
			var div4=$$('<div class="map_bus_div4"><label>首末班时间：'+LineInfo.line.lineUpStartTime+'-'+LineInfo.line.lineUpEndTime+'&nbsp;全程：'+
						LineInfo.line.price+'元</label></div>');
			var div5=$$('<div class="map_bus_div5"></div>');//实时到站
			...
			'<div id="map_bus_div7_content2" class="map_bus_div7_content"><input type="text" class="input_tex6" id="map_bus_div7_cnt2"/><span class="map_bus_div7_button" onclick=transfer_Way(2,"'+lonlat+'","'+StationInfo.name+'")>公交</span><span class="map_bus_div7_button" onclick=transfer_Way(4,"'+lonlat+'","'+StationInfo.name+'")>自驾</span></div>'+
			'<div id="map_bus_div7_content3" class="map_bus_div7_content"><input type="text" class="input_tex6" id="map_bus_div7_cnt3"/><span class="map_bus_div7_button" onclick="searchKey_Way()">搜索</span></div></div>';
			var popup=map.createPopup(lonlat,500,460,html);
			$$('#poupcontent').empty().append(div3);
			$$('#poupcontent').append(div4);
			$$('#poupcontent').append(div5);
			$$('#poupcontent').append(content);
...
function LeftPoup(num,stationId,lineId,lonlat,direct){
	...
	var icon = map.createIcon("WebSit/image/map_icon/"+(num+1)+".png",26);
	...
	$$.ajax( {
		...
		success : function(data) {
			if(direct==1){
				...
				var div3=$$('<div class="map_bus_div3"><label class="poupcontent_busRouteName">' + LineInfo.line.name + '</label>'+
							'<div class="map_bus_div3_l1">'+LineInfo.line.upEndStationName+'<img src="WebSit/image/common/busStation_srart_end.png"/>'+
							LineInfo.line.upStartStationName+'</div><div class="map_bus_div3_l2"><img src="WebSit/image/common/bus_pass.png"/>途中'+
							'<img src="WebSit/image/common/refresh.png" onclick="QueryOriginBusArrive()" style="cursor: pointer;" />刷新</div></div>');
				var div4=$$('<div class="map_bus_div4"><label>首末班时间：'+LineInfo.line.lineUpStartTime+'-'+LineInfo.line.lineUpEndTime+'&nbsp;全程：'+
							LineInfo.line.price+'元</label></div>');
				var div5=$$('<div class="map_bus_div5"></div>');//实时到站
			...
			if(direct==2){
				...
				var div3=$$('<div class="map_bus_div3"><label class="poupcontent_busRouteName">' + LineInfo.line.name + '</label>'+
							'<div class="map_bus_div3_l1">'+LineInfo.line.downEndStationName+'<img src="WebSit/image/common/busStation_srart_end.png"/>'+
							LineInfo.line.downStartStationName+'</div><div class="map_bus_div3_l2"><img src="WebSit/image/common/bus_pass.png"/>途中'+
							'<img src="WebSit/image/common/refresh.png" onclick="QueryOriginBusArrive()" style="cursor: pointer;" />刷新</div></div>');
				var div4=$$('<div class="map_bus_div4"><label>首末班时间：'+LineInfo.line.lineUpStartTime+'-'+LineInfo.line.lineUpEndTime+'&nbsp;全程：'+
							LineInfo.line.price+'元</label></div>');
				var div5=$$('<div class="map_bus_div5"></div>');//实时到站
				...
				if(data!=""){
					...
					if(list!=""){
						for(var i=0;i<list.length;i++){
							...
							$$("#poupcontent #busicon"+linum).html("<img src='WebSit/image/common/bus_pass.png'/>");
						}
					}else{
