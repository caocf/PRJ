        ...
        success: function (data) {
            ...
            if (obj != null && obj.length > 0) {
                ...
                $(obj).each(function (index, item) {
                    ...
                    var tdspan = "";
                    if (app == '待审核') {
                        tdspan = "<td class='td' style='color:#f89513 '>" + app + "</td><td class='td'><span style='cursor: pointer;'  onclick='GoShow(" + item.id + ")'>查看</span></td>";
                    } else {
                        var r = item.isllegal;
                        if (r == '不构成违章') {
...
function GoShow(id) {
    window.location.href = $('#basePath').val() + 'zfcx_detail?id=' + id + '&flag=0';
}
