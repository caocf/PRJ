$(document).ready(function () {
    ...
    var flag = $("#flag").val();
    if (flag == '1') {
        $("#jycx_li").addClass("active");
    } else if (flag == '0') {
        $("#ybcx_li").addClass("active");
    } else {
...
function init() {
    ...
    $.ajax({
        ...
        success: function (data) {
            var obj = data.map.baseen;
            var pojo = data.map.penaltyen;
            ...
            if (isnull(obj) != '') {
                ...
                if (flag == '1') {
                    ...
                    $("#trnumber").show();
                    if (pojo != null && pojo != '') {
                        $("#sdepend").text(pojo.depend + " 第" + pojo.item1 + "条、第" + pojo.item2 + "款、第" + pojo.item3 + "项");
                        $("#ssuggest").text(pojo.suggest);
                        $("#snumber").text(pojo.number);
                    }
                }
                ...
                if (pojo != null && pojo != '') {
                    var signeden = pojo.signedpdfENSet;
                    var pojoid = pojo.id;
                    if (signeden != null && signeden != '' && signeden.length > 0) {
                        var pdfpath = signeden[0].dir;
                        var pdffir = '';
                        if (pdfpath == null || pdfpath == '' || pdfpath == undefined) {
                            $("#signpdfdiv").show();
                            pdffir = '<div class="imagediv"><div onclick="showpdf(' + pojoid + ',\'' + signeden[0].unsigneddir + '\')"><img style="width: 100%;height: 100%" src="image/ship/pdf.png"></div></div>';
                        } else {
                            $("#signpdfdiv").hide();
                            pdffir = '<div class="imagediv"><div onclick="showpdf(' + pojoid + ',\'' + pdfpath + '\')"><img style="width: 100%;height: 100%" src="image/ship/pdf.png"></div></div>';
                        }
                        $("#evidencediv").append($(pdffir));
                    }
                }
