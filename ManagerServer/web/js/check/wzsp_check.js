                ...
                if (evidences != null && evidences.length > 0) {
                    for (var i in evidences) {
                        ...
                        if (ext == 'mp3' || ext == 'acc' || ext == 'amr') {
                            ...
                            divobj = '<div class="imagediv"><div onclick="lawdownload(' + fj.id + ')"><img style="width: 100%;height: 100%" src="' + fj.dir + '"></div></div>';
                        }
...
function appro() {
    ...
    var isillegalBoolean = $("input[name='radiobutton']:checked").val();
    var descr = $("#shenheword").val();
    $.ajax({
        url: 'postcheck',
        type: "post",
        dataType: "json",
        data: {
            "id": id,
            "isillegalBoolean": isillegalBoolean,
            "descr": descr,
            "checker": checker
        },
        success: function (data) {
...
function shipOrbitmapmake() {
    shipOrbitmap = new WebtAPI.WMap($$("map_div"));
