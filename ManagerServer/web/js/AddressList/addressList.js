		...
		var Statue1="",Statue2="",Statue3="";
			for(var i=0;i<data.list.length;i++)
			{
				Statue3=Statue3+"<input type='radio' value='"+data.list[i].statusId+"' name='statusId' " +
						" onclick='onchecked("+data.list[i].statusId+")'/>"+data.list[i].statusName;
...
function appendToTable(userlist, listLength) {
	...
	for ( var i = 0; i < listLength; i++) {
		...
		//newTr.append($("<td>" + userlist[i][0].userName + "</td>"));
		...
		newTr.append($("<td><a class='operation' onclick='addresssee("+ userlist[i][0].userId + ")'>查看</a>"
						+ "</td>"));
...
function addresssee(userId){
	window.location.href=$("#basePath").val()+"page/AddressList/addressSee.jsp?UserId="	+userId;
}
