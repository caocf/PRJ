...
function CreateReport() {
	...
	$("#report_imgdiv").append($("<img src='"+$("#basePath").val()+"ShowElectImage?reportModel.beginTime="+$("#year_select").val()+"&" +
			"reportModel.endTime="+$("#partoftime_select").val()+"' />"));
	$(".parttime").text($("#year_select").val());	
	$(".contentRange").text("年度船舶航行电子报告统计");
	$.ajax( {
		url : 'ReportOfElectReport',
		type : "post",
		dataType : "json",
		data : {
...
function GetExcel(){
	window.location.href=$("#basePath").val()+"downloadElectExcel?reportModel.beginTime="+$("#year_select").val()+"&" +
			"reportModel.endTime="+$("#partoftime_select").val();
}
