	...
	$.ajax( {
		...
		if(data.list!=null){
			for(var i=0;i<data.list.length;i++){
				document.getElementById("condition_select").options
				.add(new Option(data.list[i],
						data.list[i]));
			}	
		}
...
function CreateReport() {
	...
	$("#report_imgdiv").append($("<img src='"+$("#basePath").val()+"ShowDraftImage?reportModel.beginTime="+$("#beginTime").val()+"&" +
			"reportModel.endTime="+endTime1+"&reportModel.oneName="+$("#condition_select").val()+"' />"));
	...
	$.ajax( {
		...
		success : function(data) {
			...
			for(var i=1;i<data.list.length;i++){
				...
				newTr.append($("<tr class='addTr'><td></td><td style='border: solid 1px #ccc;' colspan=3>无相关数据</td></tr>"));
...
function GetExcel(){
	...
	if(endTime=="")
		endTime=$("#endTime").val();
	var endTime1=$("#endTime").val()+" 23:59:59";
	window.location.href=$("#basePath").val()+"downloadDraftExcel?reportModel.beginTime="+$("#beginTime").val()+"&" +
			"reportModel.endTime="+endTime1+"&reportModel.oneName="+$("#condition_select").val();
}
