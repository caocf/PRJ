function pagingmake(actionname,functionname,page,pages){
	...
	var isfirstonclick="onclick="+functionname+"(1)";//首页点击事件
	var ispreviousonclick="onclick="+functionname+"('"+shang+"')";//上一页点击事件
	if(parseInt(page)==1){
	...
	var islastonclick="onclick="+functionname+"('"+parseInt(pages)+"')";
	var isnextonclick="onclick="+functionname+"('"+xia+"')";
	...
	if(parseInt(pages)<=5){
		...
		for (var int = 1; int <= parseInt(pages); int++) {
			...
			var intpage="<li "+active+"><a onclick="+functionname+"('"+int+"')>"+int+"</a></li>";
