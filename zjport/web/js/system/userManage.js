            ...
            if(data.resultcode==-1){
                ...
                if(list==""){
                    ...
                    for(var int=0;int<list.length;int++){
                        ...
                        node={"id":list[int].stOrgId, "name":"<div onclick=showUser('"+list[int].stOrgId+"',1,'"+list[int].stOrgName+"')><span><i class='fa fa-home'></i>&nbsp;"+list[int].stOrgName+"</span></div>","pId":pid,"phone":0,'titleword':list[int].stOrgName};
                        nbnodes.push(node);
                    }
                    if(type!=""&&type!=null) {
                        showAllUser();
...
function appendToTable(array){
    ...
    for(var i=0;i<array.length;i++){
        ...
        //操作
        newTr.append($("<td class='center'>&nbsp;&nbsp;<a class='clickword' href='showDetail?id="+array[i][4]+"'>查看</a>&nbsp;&nbsp;</td>"));
...
function showDetail(userId,type) {
    ...
    $.ajax({
        type : "POST",
        url : "../officeAssistant/showUserDetail",
        datatype : "json",
        data : {'userId':userId},
        success : function(data) {
