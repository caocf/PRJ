...
$(document).ready(function () {
    ...
    showSearchDiv(1);
    $("#selhangdao").empty();
    $("#selhangdao").append($('<option value="-1">全部航道</option>'));
    initHangdao();
});
        ...
        if (ifResultOK(data)) {
            ...
            if (records) {
                ...
                if (!hasPerm("VIEW_SHENHDAO")) {
                    $("#selhdaosjbh").attr("disabled", "disabled");
                }
            }
        }
    });
...
function initHangdao() {
    ...
    queryhangdao(-1,xzqh, function (hdaolist) {
        ...
        for (var i = 0; i < hdaolist.length; i++) {
            ...
            var name = hdao.hdmc;
            data[id] = name;
            $("#selhangdao").append($('<option value="' + id + '">' + name + '</option>'));
        }
        $("#divhangdao").empty();
        $("#divhangdao").addselect({
            data: data,
            validators: {
                notempty: {
                    msg: '请选择航道'
...
function initHangduan(xzqh) {
    $("#temphangduan").hide();
    // 加载航道选择
    ajax('hangduan/queryhangduanbysshdid', {
        'sshdid': $("#selhangdao").val(),
        'xzqh': xzqh
    }, function (data) {
        if (ifResultOK(data)) {
            var records = getResultObj(data);
