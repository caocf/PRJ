...
<head>
    ...
    <script>
        function loadbaobiao() {
            ...
            $('#baobiaoiframe').attr(
                    "src",
                    "statistics/querylcfltable?loginid=" + $("#userid").val()
                    + "&starttime=" + day + "&xzqh=" + xzqh);
        }
        function exportlcfltable() {
            var day = $("#selday").attr('realvalue');
            var xzqh = $("#selxzqh").attr('selitem');
            window.location.href = $("#basePath").val() + "statistics/exportlcfltable?loginid=" + $("#userid").val() + "&starttime=" + day + "&xzqh=" + xzqh;
        }
    </script>
...
<div class="container-fluid  shadow border" style="background:white;">
    <div class="row navcontenttitle borderbottom">
        <div class="col-xs-12">航道里程分类统计表</div>
    </div>
        ...
        <c:if test="${ui.hasPerm('EXPORT_BB') || ui.hasPerm('PRINT_BB')}">
            <div class="col-xs-2">
                <c:if test="${ui.hasPerm('EXPORT_BB')}">
                    <button class="btn btn-default" onclick="exportlcfltable()">导出</button>
                </c:if>
