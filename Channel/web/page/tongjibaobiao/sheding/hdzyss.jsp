...
<head>
    ...
    <script>
        function loadbaobiao() {
            ...
            $('#baobiaoiframe').attr(
                    "src",
                    "statistics/querysstable?loginid=" + $("#userid").val()
                    + "&starttime=" + day + "&xzqh=" + xzqh);
        }
        function exportsstable() {
            var day = $("#selday").attr('realvalue');
            window.location.href = $("#basePath").val() + "statistics/exportsstable?loginid="
                    + $("#userid").val() + "&starttime=" + day;
        }
    </script>
...
<div class="container-fluid  shadow border" style="background:white;">
    <div class="row navcontenttitle borderbottom">
        <div class="col-xs-12">航道主要设施汇总表</div>
    </div>
        ...
        <c:if test="${ui.hasPerm('EXPORT_BB') || ui.hasPerm('PRINT_BB')}">
            <div class="col-xs-2">
                <c:if test="${ui.hasPerm('EXPORT_BB')}">
                    <button class="btn btn-default" onclick="exportsstable()">导出</button>
                </c:if>
