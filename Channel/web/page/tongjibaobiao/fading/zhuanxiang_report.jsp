...
<head>
    ...
    <script>
        function loadbaobiao() {
            ...
            var dptflag = 0;
            if (hasPerm('VIEW_SHENZXGCBB') || hasPerm('VIEW_SHIZXGCBB'))
                dptflag = 1;
            else
                dptflag = 0;
            ...
            if (starttime != null && starttime != '' && endtime != null
                ...
                $('#baobiaoiframe').attr(
                        "src",
                        "statistics/queryzxgctable?loginid=" + $("#userid").val()
                        + "&flag=" + type + "&gldw=" + gldw + "&starttime=" + starttime
                        + "&endtime=" + endtime + "&dptflag=" + dptflag);
            }
        }

        function exportzxgctable() {
            var type = $('#selecttimetype').val();
            var gldw = $('#selgldw').attr('selitem');
            var starttime = '';
            var endtime = '';
            ...
            if (starttime != null && starttime != '' && endtime != null
                ...
                window.location.href = $("#basePath").val()
                        + "statistics/exportzxgctable?loginid=" + $("#userid").val()
                        + "&flag=" + type + "&gldw=" + gldw + "&starttime=" + starttime + "&endtime="
                        + endtime + "&dptflag=" + dptflag;
            }
...
<div class="container-fluid shadow border" style="background:white;">
    <div class="row navcontenttitle borderbottom">
        <div class="col-xs-12">专项养护工程完成情况工作统计表</div>
    </div>
    <div class="row borderbottom" style="padding:10px 0 10px 0;">
        <div class="col-xs-1 text-right" style="margin-top:8px;">管理单位</div>
        <div class="col-xs-2 text-center" id="divselgldw">
            <script>
                <c:if test="${ui.hasPerm('VIEW_SHENZXGCBB')}">
                ...
                <c:if test="${ui.hasPerm('VIEW_SHIZXGCBB')}">
                $("#divselgldw").addseldpt({
                    id: 'selgldw',
                    selectfn: function () {
                        loadbaobiao();
                ...
                <c:if test="${ui.hasPerm('VIEW_DPTZXGCBB')}">
                $("#divselgldw").addseldpt({
                    id: 'selgldw',
                    selectfn: function () {
                        loadbaobiao();
        ...
        <div class="col-xs-4 hide" id="selquarterdiv">
            <select class="form-control"
                ...
                <script>
                    ...
                    ajax(
                            'maintenance/queryzxgcstarttime',
                            {
                                'loginid': $("#userid").val()
                            },
                            function (result) {
        ...
        <c:if test="${ui.hasPerm('EXPORT_BB') || ui.hasPerm('PRINT_BB')}">
            <div class="col-xs-2">
                <c:if test="${ui.hasPerm('EXPORT_BB')}">
                    <button class="btn btn-default" onclick="exportzxgctable()">导出</button>
                </c:if>
