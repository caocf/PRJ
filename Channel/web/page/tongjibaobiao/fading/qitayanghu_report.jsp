...
<head>
    ...
    <script>
        function loadbaobiao() {
            ...
            var dptflag = 0;
            if (hasPerm('VIEW_SHENZXYHBB') || hasPerm('VIEW_SHIZXYHBB'))
                dptflag = 1;
            else
                dptflag = 0;
        ...
        function exportzxtable() {
            ...
            if (starttime != null && starttime != '' && endtime != null
                ...
                window.location.href = $("#basePath").val()
                        + "statistics/exportzxtable?loginid=" + $("#userid").val() + "&flag="
...
<div class="container-fluid shadow border" style="background:white;">
    <div class="row navcontenttitle borderbottom">
        <div class="col-xs-12">全航区其他航道例行养护工作统计表</div>
    </div>
    <div class="row borderbottom" style="padding:10px 0 10px 0;">
        ...
        <div class="col-xs-2 text-center" id="divseldw">
            <script>
                <c:if test="${ui.hasPerm('VIEW_SHENZXYHBB')}">
                $("#divseldw").addseldpt({
                    id: 'seldw',
                    defaultval: 1,
                    selectfn: function () {
                ...
                <c:if test="${ui.hasPerm('VIEW_SHIZXYHBB')}">
                $("#divseldw").addseldpt({
                    id: 'seldw',
                    defaultval: <%=szshiju%>,
                    selectfn: function () {
                ...
                <c:if test="${ui.hasPerm('VIEW_DPTZXYHBB')}">
                $("#divseldw").addseldpt({
                    id: 'seldw',
                    defaultval: <%=szju%>,
                    selectfn: function () {
        ...
        <c:if test="${ui.hasPerm('EXPORT_BB') || ui.hasPerm('PRINT_BB')}">
            <div class="col-xs-2">
                <c:if test="${ui.hasPerm('EXPORT_BB')}">
                    <button class="btn btn-default" onclick="exportzxtable()">导出</button>
                </c:if>
