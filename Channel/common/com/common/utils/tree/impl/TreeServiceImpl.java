...
public abstract class TreeServiceImpl<E extends TreeNode, R extends TreeNodeRelation>
     ...
     * 以递归的方式所有子节点
     */
    private List<E> _findParentNodes_r(E nd, String name) {
        List<E> childs = new ArrayList<>();

        // 添加符合条件的一级子节点
        childs.addAll(findParentNodes(nd, name).getData());

        // 查询子节点的所有子节点
        List<E> listsall = findParentNodes(nd).getData();
        for (int i = 0; i < listsall.size(); i++) {
            childs.addAll(_findParentNodes_r(listsall.get(i), name));
        }
        return childs;
    }
     ...
     * 以递归的方式所有子节点
     */
    private List<E> _findParentNodes_r(E nd, String name, int type) {
        List<E> childs = new ArrayList<>();

        // 添加符合条件的一级子节点
        childs.addAll(findParentNodes(nd, name, type).getData());

        // 查询子节点的所有子节点
        List<E> listsall = findParentNodes(nd).getData();
        for (int i = 0; i < listsall.size(); i++) {
            childs.addAll(_findParentNodes_r(listsall.get(i), name, type));
        }
        return childs;
    }
     ...
     * 以递归的方式所有子节点
     */
    private List<E> _findParentNodes_r(E nd, int type) {
        List<E> childs = new ArrayList<>();

        // 添加符合条件的一级子节点
        childs.addAll(findParentNodes(nd, type).getData());

        // 查询所有子节点的所有符合条件子节点
        List<E> listsall = findParentNodes(nd).getData();
        for (int i = 0; i < listsall.size(); i++) {
            childs.addAll(_findParentNodes_r(listsall.get(i), type));
        }
        return childs;
    }
     ...
     */
    @Override
    public E findNode(E node, int type) {
        return (E) getTreeDao()._findNode(node, type);
    }
     ...
     */
    @Override
    public E findNode(E node, int type, String name) {
        return (E) getTreeDao()._findNode(node, type, name);
    }
     ...
     */
    @Override
    public E findNode(E node, String name) {
        return (E) getTreeDao()._findNode(node, name);
    }
    ...
    public List<E> findParentNodes_r(E nd, int type) {
        List<E> lists = _findParentNodes_r(nd, type);
    ...
    public List<E> findParentNodes_r(E nd, String name) {
        List<E> lists = _findParentNodes_r(nd, name);
    ...
    public List<E> findParentNodes_r(E nd, int type, String name) {
        List<E> lists = _findParentNodes_r(nd, name, type);
    ...
    @Override
    public int countChildrenNode(E node) {
        return getTreeDao()._countChildrenNode(node);
    }
}
