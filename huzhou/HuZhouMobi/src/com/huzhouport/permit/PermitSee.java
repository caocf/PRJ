...
public class PermitSee extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		bt_gotoupdate.setOnClickListener(new OnClickListener() {
			...
			@Override
			public void onClick(View v) {
				Intent intent = new Intent(PermitSee.this, PermitUpdate.class);
	...
	public void getEvidenceDate(String result) {
		...
		try {
			...
			Toast.makeText(PermitSee.this, "补充信息获取失败", Toast.LENGTH_SHORT)
					.show();
			e.printStackTrace();
		}
	}
	...
	class GetBitmapList extends AsyncTask<Void, Void, Void> {
		...
		public GetBitmapList(Context context) {
			...
			pDialog = new ProgressDialog(PermitSee.this);
			pDialog.setTitle("图片");
		...
		protected void onPostExecute(Void result) {
			ma = new MyAdapter(PermitSee.this, lsmap);
			gv2.setAdapter(ma);
...
class GetLocation extends AsyncTask<Void, Void, String>{
	...
	public void getLocationFromService(String result) {
		...
		try {
			...
			Toast.makeText(PermitSee.this, "位置信息获取失败", Toast.LENGTH_SHORT)
					.show();
			e.printStackTrace();
		}
	}
	...
	class gridViewOnClickListener2 implements OnItemClickListener {
		...
		public void onItemClick(AdapterView<?> arg0, View arg1, int arg2,
			...
			final int N = arg2;
			new AlertDialog.Builder(PermitSee.this)
					.setTitle("查看图片")
	...
	class Download extends AsyncTask<String, Void, Integer> {
		...
		protected void onPostExecute(Integer result) {
			if(result==0 || result ==1)
				ShowImage.ShowImageBySystemApp(imageURL, PermitSee.this);
			
//			if (result == 0)
//				Toast.makeText(PermitSee.this, GlobalVar.FILE_DOWNLOAD_SUCCESS,
//						Toast.LENGTH_SHORT).show();
//			if (result == 1)
//				Toast.makeText(PermitSee.this, "文件已存在", Toast.LENGTH_SHORT)
//						.show();
			if (result == -1)
				Toast.makeText(PermitSee.this, "下载失败", Toast.LENGTH_SHORT)
						.show();
			super.onPostExecute(result);
		}
	...
	public void initDate(String result) {
		...
		try {
			...
			for (int i = 0; i < jsonArray.length(); i++) {
				...
				JSONObject jsonObject1 = (JSONObject) jsonArray2.opt(0);
				JSONObject jsonObject2 = (JSONObject) jsonArray2.opt(1);
				tv_title.setText(jsonObject1.getString("inspectionObject"));
			...
			Toast.makeText(PermitSee.this, "没有搜索到相关数据", Toast.LENGTH_SHORT)
					.show();
			e.printStackTrace();
		}
...
class GetCheckName extends AsyncTask<Void, Void, String>{
	...
	public void getCheckName(String result2) {
		...
		try {
			...
			Toast.makeText(PermitSee.this, "审核信息获取失败", Toast.LENGTH_SHORT)
					.show();
			e.printStackTrace();
		}
	}
