	...
	class GetInspectionList extends AsyncTask<Void, Void, String>
	{
		...
		protected String doInBackground(Void... params)
		{
			...
			if (queryCondition.length() != 0)
			{
				...
				{
					Toast.makeText(PermitSearch.this, "搜索内容失败",
		...
		protected void onPostExecute(String result)
		{
			...
			{	moreView.setVisibility(View.GONE);
				lv.removeFooterView(moreView); // 移除底部视图
				//Toast.makeText(PermitSearch.this, "已加载全部数据", 3000).show();
			}
	...
	public void showInspectionList(String result)
	{
		...
		try
		{
			...
			for (int i = 0; i < jsonArray.length(); i++)
			{
				...
				int nodeKind = jsonObject3.getInt("insCategoriese");// 1:港政2：运政3：航政4：海事
				switch (nodeKind)
				{
					case 1:
						map.put("object",
		...
		{
			Toast.makeText(PermitSearch.this, "没有搜索到相关数据", Toast.LENGTH_SHORT)
					.show();
			e.printStackTrace();
		}
	}
