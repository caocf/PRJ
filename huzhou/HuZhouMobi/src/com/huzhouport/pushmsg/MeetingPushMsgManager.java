 ...
 * 
 */
public class MeetingPushMsgManager {
	...
	public int getUnPushedMsg(int userid) {
		...
		String url = HttpUtil.BASE_URL + "queryUnPushedMsg?onlysize=true&pushMsg.modulerid="
				+ PushMsg.MODULERID_MEETING + "&pushMsg.userid="+userid;
		...
		try {
			...
			Log.d("MeetingPushMsgManager", "getUnPushedMsg Size: "+size);
		} catch (Exception e) {
			e.printStackTrace();
		}
		return size;
	...
	public int getPushedUnReadMsgCnt(int userid) {
		...
		try {
			...
			Log.d("MeetingPushMsgManager", "getPushedUnReadMsgCnt Size: "+size);
		} catch (Exception e) {
			e.printStackTrace();
		}
