 ...
 * 
 */
public class IllegalPushMsgManager {
	...
	public int getUnPushedMsg(int userid) {
		...
		String url = HttpUtil.BASE_URL + "queryUnPushedMsg?onlysize=true&pushMsg.modulerid="
				+ PushMsg.MODULERID_ILLEGAL + "&pushMsg.userid="+userid;
		...
		try {
			...
			Log.d("IllegalPushMsgManager", "getUnPushedMsg Size: "+size);
		} catch (Exception e) {
			e.printStackTrace();
		}
		return size;
	...
	public int getPushedUnReadMsgCnt(int userid) {
		...
		try {
			...
			Log.d("IllegalPushMsgManager", "getPushedUnReadMsgCnt Size: "+size);
		} catch (Exception e) {
			e.printStackTrace();
		}
