...
public class ScheduleAddTime extends Activity 
{
	...
	class finishAdd implements OnClickListener {
		...
		public void onClick(View v) {
			if (et_name.getText().length() == 0) {
				Toast.makeText(ScheduleAddTime.this, "请输入日程名称",
	...
	class Post extends AsyncTask<Void, Void, String> 
	{
		...
		{
			pDialog = new WaitingDialog().createDefaultProgressDialog(ScheduleAddTime.this, this,"提交中,请稍候···");
			pDialog.show();
		}
		...
		protected String doInBackground(Void... params)
		{
			...
			UploadActivity.tool.addFile("添加" + et_name.getText() + "的日程安排",
			 ...
			 try 
			 { 
				 String resultString=hfu.post(actionUrl, paramsDate, files, "scheduleAttachment.af");
				 return resultString;
			
			  }
			 catch (IOException e) 
			 { 
				 Toast.makeText(ScheduleAddTime.this,"发生错误", Toast.LENGTH_SHORT) .show(); e.printStackTrace();
				 return null;
			 }
		...
		protected void onPostExecute(String result) 
		{
			 ...
			 * "保存成功", Toast.LENGTH_SHORT) .show(); Intent intent = new
			 * Intent(ScheduleAddTime.this, CalendarActivity.class);
			 * intent.putExtra("User", user); startActivity(intent); finish(); }
			 * else Toast.makeText(ScheduleAddTime.this, "保存失败",
			 * Toast.LENGTH_SHORT) .show(); } catch (JSONException e) {
			 * Toast.makeText(ScheduleAddTime.this, "发生错误", Toast.LENGTH_SHORT)
			...
			if (user != null) 
			{
				new Log(user.getName(), "添加日程安排", GlobalVar.LOGSAVE, "").execute();
			}
			if (pDialog != null)
				pDialog.dismiss();
	...
	class ShowDate implements OnClickListener {
		...
		public void onClick(View v) {
			 ...
			 * 
			 * 
			 * if (year < 1901 || year > 2049) { // 不在查询范围内 new
			 * AlertDialog.Builder(ScheduleAddTime.this) .setTitle("错误日期")
			 * .setMessage("跳转日期范围(1901/1/1-2049/12/31)")
			 * .setPositiveButton("确认", null).show(); } else { timeList[0] =
	...
	class getScheduleKindList extends AsyncTask<Void, Void, String> {
		 ...
		 * ProgressDialog .show(context, "数据加载", "数据加载中，请稍候···", true); }
		 */
		@Override
		protected String doInBackground(Void... params) {
		...
		protected void onPostExecute(String result) {
			...
			try {
				...
				Toast.makeText(ScheduleAddTime.this, "数据获取失败",
						Toast.LENGTH_SHORT).show();
				e.printStackTrace();
			}
