...
public class ScheduleUpdateTime extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		((TextView)findViewById(R.id.schedule_update_title)).setText("编辑会议");
		createRemind();
		...
		((TableRow)findViewById(R.id.schedule_update_tr_kind)).setVisibility(View.GONE);
		imguser.setOnClickListener(new OnClickListener() {

			public void onClick(View v) {
				Intent intent = new Intent(ScheduleUpdateTime.this,
	...
	class addFile implements OnClickListener {
		...
		public void onClick(View v) {
			...
			intent.setClass(ScheduleUpdateTime.this, GetSDTreeActivity.class);
	...
	class gotoSee implements OnClickListener {
		...
		@Override
		public void onClick(View v) {
			Intent intent = new Intent(ScheduleUpdateTime.this, ScheduleSeeTime.class);
	...
	class UploadDate extends AsyncTask<Void, Void, String> {
		...
		public UploadDate() {
			...
			pDialog = new WaitingDialog().createDefaultProgressDialog(ScheduleUpdateTime.this, this);
			pDialog.show();	
		}
		@Override
		protected String doInBackground(Void... params) {
			...
			if (et_name.getText().length() == 0) {
				Toast.makeText(ScheduleUpdateTime.this, "请输入会议名称",
				...
				UploadActivity.tool.addFile("修改"+et_name.getText()+"的会议",actionUrl, paramsDate,files, "scheduleAttachment.af");
			}
			return null;
		}
		...
		protected void onPostExecute(String result) {
			...
			/*if (result != null) {
				...
				try {
					...
					if (jsonObject0.equals("1")) {
						Toast.makeText(ScheduleUpdate.this, "修改成功",
								Toast.LENGTH_SHORT).show();
						...
						Toast.makeText(ScheduleUpdate.this, "修改失败",
								Toast.LENGTH_SHORT).show();
				} catch (JSONException e) {
					Toast.makeText(ScheduleUpdate.this, "发生错误",
							Toast.LENGTH_SHORT).show();
					e.printStackTrace();
				}
			...
			if(user!=null){
			    new Log(user.getName(), "修改会议",GlobalVar.LOGUPDATE, "").execute();
			}
			super.onPostExecute(result);
		}
	...
	class ShowDate implements OnClickListener {
		...
		public void onClick(View v) {
			SelectPicPopupWindow menuWindow = new SelectPicPopupWindow(
					ScheduleUpdateTime.this, v, tv_day);
			menuWindow.showAtLocation(
					v,Gravity.BOTTOM | Gravity.CENTER_HORIZONTAL, 0, 0); // 设置layout在PopupWindow中显示的位置
	...
	public void getScheduleKind(String result) {
		...
		try {
			...
			Toast.makeText(ScheduleUpdateTime.this, "数据获取失败", Toast.LENGTH_SHORT)
					.show();
			e.printStackTrace();
		}
	...
	class spinner2Listener implements OnItemSelectedListener {
		...
		public void onItemSelected(AdapterView<?> arg0, View arg1, int arg2,
				long arg3) {
			if (toastNum != -1)
				Toast.makeText(ScheduleUpdateTime.this,
	...
	public void dialog_download() {
		new AlertDialog.Builder(this).setTitle("删除原有文档").setMessage("删除该文档？")
	...
	class DeleteAttachment extends AsyncTask<Void, Void, String> {
		...
		protected void onPostExecute(String result) {
			...
			try {
				...
				String jsonObject0 = data.getString("ieventId");
				if (jsonObject0.equals("1"))
					Toast.makeText(ScheduleUpdateTime.this, "删除成功",
							Toast.LENGTH_SHORT).show();
				old_files.remove(at_name[wzpost]);
				creatGV();
			} catch (Exception e) {
				Toast.makeText(ScheduleUpdateTime.this, "发生错误", Toast.LENGTH_SHORT)
						.show();
				e.printStackTrace();
			}
	...
	public void dialog_down(String kt) {
		final String kt2 = kt;
		new AlertDialog.Builder(this).setTitle("移除文件").setMessage("移除该文件？")
				.setPositiveButton("是", new DialogInterface.OnClickListener() {
