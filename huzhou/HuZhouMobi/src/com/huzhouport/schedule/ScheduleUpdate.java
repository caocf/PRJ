...
public class ScheduleUpdate extends Activity {
	...
	class UploadDate extends AsyncTask<Void, Void, String> {
		...
		protected String doInBackground(Void... params) {
			...
			if (et_name.getText().length() == 0) {
				Toast.makeText(ScheduleUpdate.this, "请输入日程名称",
				...
				UploadActivity.tool.addFile("修改"+et_name.getText()+"的日程",actionUrl, paramsDate,files, "scheduleAttachment.af");
			}
			return null;
		}
		...
		protected void onPostExecute(String result) {
			...
			if(user!=null){
			   new Log(user.getName(), "修改日程安排",GlobalVar.LOGUPDATE, "").execute();
			}
			super.onPostExecute(result);
		}
	...
	public void getScheduleKind(String result) {
		...
		try {
			...
			Toast.makeText(ScheduleUpdate.this, "数据获取失败", Toast.LENGTH_SHORT)
					.show();
			e.printStackTrace();
		}
	...
	class DeleteAttachment extends AsyncTask<Void, Void, String> {
		...
		protected void onPostExecute(String result) {
			...
			try {
				...
				String jsonObject0 = data.getString("ieventId");
				if (jsonObject0.equals("1"))
					Toast.makeText(ScheduleUpdate.this, "删除成功",
							Toast.LENGTH_SHORT).show();
				old_files.remove(at_name[wzpost]);
				creatGV();
			} catch (Exception e) {
				Toast.makeText(ScheduleUpdate.this, "发生错误", Toast.LENGTH_SHORT)
						.show();
				e.printStackTrace();
			}
