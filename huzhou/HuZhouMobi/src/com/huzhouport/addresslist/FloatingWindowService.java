...
public class FloatingWindowService extends Service {
	...
	public void onCreate() {
		super.onCreate();
		System.out.println("11111234566");
		...
		public void onCallStateChanged(int state, String incomingNumber) {
                switch(state){
                        case TelephonyManager.CALL_STATE_RINGING :
                            ...
                            System.out.println("已经监听到来电123");
                            Cursor cursor = db.rawQuery("SELECT name,departmentName FROM mm_User where tel = ?", new String[]{incomingNumber});  
                            if( cursor != null ){
                                if( cursor.moveToFirst() ){
                                	System.out.println("已经监听到来电124");
                            		//Toast.makeText(getApplicationContext(), cursor.getString(0), Toast.LENGTH_LONG).show();
                            		//调用创建窗口
                            		Log.e("PhoneService", "监听到来电，号码为"+cursor.getString(0)+" "+cursor.getString(1));
