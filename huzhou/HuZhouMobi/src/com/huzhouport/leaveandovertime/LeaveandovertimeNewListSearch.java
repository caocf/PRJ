...
public class LeaveandovertimeNewListSearch extends Activity {
		...
		public void onClick(View v) {
			...
			if (content.length() == 0) {
				Toast.makeText(LeaveandovertimeNewListSearch.this,
						"请先在搜索框里输入内容", Toast.LENGTH_SHORT).show();
			} else {
				if (searchid.equals("0")) {
					ListTask task = new ListTask(
							LeaveandovertimeNewListSearch.this); // 异步
					task.execute();
	...
	class ListTask extends AsyncTask<String, Integer, String> {
		...
		protected void onPostExecute(String result) {
			...
			if (result == null || result.equals("null")) {
				Toast.makeText(LeaveandovertimeNewListSearch.this, "网络异常",
						Toast.LENGTH_SHORT).show();
			} else {
				getLeave(result);// 获得数据
				showLeave();
				// getLeave1(result);//获得数据
				showLeave1();
	...
	public void getLeave(String result) {
		...
		try {
			...
			Toast.makeText(LeaveandovertimeNewListSearch.this, "数据异常",
					Toast.LENGTH_SHORT).show();
			e.printStackTrace();
		}
	}
	...
	public void showLeave1() {
		...
		int s = l1 + l2;
		if (s == 0) {
			Toast.makeText(LeaveandovertimeNewListSearch.this, "没有找到相关数据",
					Toast.LENGTH_SHORT).show();
		}
	}
