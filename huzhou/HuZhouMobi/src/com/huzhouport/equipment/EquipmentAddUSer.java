...
public class EquipmentAddUSer extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		searchtext.addTextChangedListener(new TextWatcher() {
			...
			public void onTextChanged(CharSequence s, int start, int before, int count) {
		       ...
		       if(content.length()==0){
		    	departmentId="-1";
		    	ListTask task = new ListTask(EquipmentAddUSer.this);  // 异步
		    	task.execute();
		       }else{
	...
	public class Search implements View.OnClickListener {
		public void onClick(View v) {
			content=searchtext.getText().toString();
		       if(content.length()==0){
		    		Toast.makeText(EquipmentAddUSer.this, "请先在搜索框里输入内容", Toast.LENGTH_SHORT).show();
		       }else{
		    	   ListTask1 task = new ListTask1(EquipmentAddUSer.this);  // 异步
				    task.execute();
		       }
	...
	class ListTask extends AsyncTask<String ,Integer,String>{
		...
		protected void onPostExecute(String result) {
			...
			showuserlist();
			if(de==1&&us==1){
				Toast.makeText(EquipmentAddUSer.this, "没有数据", Toast.LENGTH_SHORT).show();
			}
	...
	public void  getdepartmentlist(String result){
			...
			try {
			 ...
			 departmentlist.add(departmentlistmap);
				...
				Toast.makeText(EquipmentAddUSer.this, "网络异常", Toast.LENGTH_SHORT).show();
				e.printStackTrace();
			}
	 }
	...
	class Userlistitem implements OnItemClickListener{
		...
		public void onItemClick(AdapterView<?> arg0, View arg1, int arg2, long arg3) {
			...
			if(userid.equals(usertv_id.getText().toString())){
				Toast.makeText(EquipmentAddUSer.this, "不能选择自己", Toast.LENGTH_SHORT).show();
			}else{
