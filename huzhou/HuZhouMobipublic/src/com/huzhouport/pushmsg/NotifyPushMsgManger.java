...
public class NotifyPushMsgManger {
	...
	private static String ORI = "public";
	 ...
	 * @return
	 */
	private static List<NotifyPushMsg> queryNonUserOriNotifyMsgAll(String ori,
			int userid, int folderid) {
		List<NotifyPushMsg> notifyPushMsgs = new ArrayList<NotifyPushMsg>();
	 ...
	 * @return
	 */
	private static int queryUserOriUnreadNotifyCnt(String ori, int userid,
			int folderid) {
		int cnt = 0;

		String url = HttpUtil.BASE_URL + "queryUserOriUnreadNotifyCnt?userid="
				+ userid + "&folderid=" + folderid + "&ori=" + ori;
		String result = HttpUtil.queryStringForPost(url);
	...
	public int getUserUnPushedUnreadNotifyCnt(User user) {
		...
		Log.d(TAG, "getUserUnPushedUnreadNotifyCnt userid:" + userid);

		PushMsgDB notifyPushMsgDB = new PushMsgDB(context);

		SQLiteDatabase db = notifyPushMsgDB.getWritableDatabase();
		// 获得目录下 面向用户的未读消息数量
		int oriuserunpushedunreadcnt = queryUserOriUnreadAndUnPushNotifyCnt(
				ORI, userid, -1);
		Log.d(TAG, "oriuserunpushedunreadcnt:" + oriuserunpushedunreadcnt);

		int nonuseroriunpushedunreadcnt = 0;
		// 同步非面向用户的通知公告消息
		List<NotifyPushMsg> notifyPushMsgs = queryNonUserOriNotifyListAll(ORI,
				userid);

		Log.d(TAG, "notifyPushMsgs size:" + notifyPushMsgs.size());
		...
		Log.d(TAG, "useroriunreadcnt:" + oriuserunpushedunreadcnt
				+ " nonuseroriunpushedunreadcnt:" + nonuseroriunpushedunreadcnt);
	...
	public int getUserUnreadNotifyCnt(User user) {
		...
		Log.d(TAG, "getUserUnreadNotifyCnt: " + userid);
		...
		Log.d(TAG, "oriuserunreadcnt:" + oriuserunreadcnt);
		// 同步非面向用户的通知公告消息
		List<NotifyPushMsg> notifyPushMsgs = queryNonUserOriNotifyListAll(ORI,
				userid);
		for (int i = 0; i < notifyPushMsgs.size(); i++) {
		...
		if (db != null)
			db.close();
		Log.d(TAG, "oriuserunreadcnt:" + oriuserunreadcnt
				+ " orinonuserunreadcnt:" + orinonuserunreadcnt);
	...
	public List<NotifyPushMsg> searchPushMsgs(User user, int folderid,
		...
		if (user != null)
			userid = user.getUserId();
		List<NotifyPushMsg> msgs = queryNotifyMsgListAllByContent(ORI, userid,
				folderid, content);
		...
		for (int i = 0; i < msgs.size(); i++) {
			...
			if (msg.getUserid() == PushMsg.USERID_BOTH
				...
				PushMsg msgg = notifyPushMsgDB.findNotifyPushMsg(db, userid,
						msg.getPushmsgid());
	...
	public List<NotifyPushMsgFolder> getFolders(User user, int folderid) {
		...
		for (int i = 0; i < folders.size(); i++) {
			...
			if (user != null)// 获得该目录下的面向用户的未读通知公告数量
				useroriunreadcnt = queryUserOriUnreadNotifyCnt(ORI, userid,
						folder.getId());
			...
			List<NotifyPushMsg> nonuseroriMsgs = queryNonUserOriNotifyMsgAll(
					ORI, userid, folder.getId());
	...
	public void readNotifyMsg(User user, NotifyPushMsg msg) {
		...
		Log.d(TAG, "readNotifyMsg: " + userid + " " + msg.getPushmsgid());
