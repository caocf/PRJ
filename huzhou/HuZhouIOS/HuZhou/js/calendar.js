(function(w){
	var today = new Date();
	w.Calendar = function (parentNode,opt) {
		// 创建日历控件基本结构	
...
//		document.body.appendChild(cldbox);
		...
		//初始化显示本月信息
		setContent(content, fistWeek, monDaynum, nowDay,true);
		...
		prevmonth.addEventListener(evt.type,function(){
			...
			dateObj.setFullYear(ddy);
		  	...
		  	setContent(content, fistWeek, monDaynum, nowDay,isToday(dateObj));
		})
	...
	var isToday = function(dateObject){
		if(getDay(today) != getDay(dateObject)){
			return false;
		}
		if(getMonth(today) != getMonth(dateObject)){
			return false;
		}
		if(getYear(today) != getYear(dateObject)){
			return false;
		}
		return true;
	}
	// 初始化日期显示方法
	function setContent(el, fistWeek, monDaynum, nowDay,isToday){
