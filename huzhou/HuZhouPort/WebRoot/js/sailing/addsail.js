...
$(document).ready(function() {
	if ($("#reportId").val() != "null") {
		$("#titlename").text("修改航行电子报告");
		showModifyInfo('ElectricReportByReportId');
	} else {
		$("#boatName").text($("#shipName").val());
	...
	$.ajax( {
		...
		success : function(data) {
			...
			if (list.reportKind == 1) {
				document.getElementById("selectsail").options.add(new Option(
						"重船航行", 1));
				document.getElementById("selectsail").options.add(new Option(
						"空船航行", 2));
			} else {
			...
			if (list.outOrIn == 1) {
				document.getElementById("selectport").options.add(new Option(
						"进港", 1));
				document.getElementById("selectport").options.add(new Option(
						"出港", 2));
			} else {
			...
			if (list.shipInfo != "") {
				...
				for ( var i = 0; i < shipmanInfo.length; i++) {
					...
					newTr.append($("<td>" + shipmanInfo[i].split(",")[0]
							+ "</td>"));
					newTr.append($("<td><label>" + shipmanInfo[i].split(",")[1]
							+ "</label><label>&nbsp;(&nbsp;"
							+ shipmanInfo[i].split(",")[2] + ":"
							+ shipmanInfo[i].split(",")[3]
							+ "&nbsp;)</label></td>"));
	...
	$.ajax( {
		...
		success : function(data) {
			...
			for ( var i = 0; i < list.length; i++) {
				var newTr = $("<tr class='addTr'></tr>");
				newTr.append($("<td>" + list[i][1].boatmanKindName + "</td>"));
				newTr
						.append($("<td><label>" + list[i][0].boatmanName
								+ "</label><label>&nbsp;(&nbsp;"
								+ list[i][2].cardName + ":"
								+ list[i][0].boatCardNum
								+ "&nbsp;)</label></td>"));
				selectboatmanInfo[i] = new Array();
				selectboatmanInfo[i][0] = list[i][1].boatmanKindName;
				selectboatmanInfo[i][1] = list[i][0].boatmanName;
				selectboatmanInfo[i][2] = list[i][2].cardName;
				selectboatmanInfo[i][3] = list[i][0].boatCardNum;
				selectboatmanInfo[i][4] = 1;
				$("#sailboatTable").append(newTr);
			}
...
function ShowBoatUserInfo() {
	$.ajax( {
		...
		success : function(data) {
			...
			var list = data.list;/* alert(JSON.stringify(list)); */
			for ( var i = 0; i < list.length; i++) {
				var newLi = $("<li class='addLi' id='selectBotman" + i
						+ "' onclick=boatmanInfoSelected(this,'" + i
						+ "')><span class='spanStyle' id='boatmanKindName"
						+ list[i][0].boatmanID + "'>"
						+ list[i][1].boatmanKindName + "</span>"
						+ "<span class='spanStyle' id='boatmanName"
						+ list[i][0].boatmanID + "'>" + list[i][0].boatmanName
						+ "</span></br>"
						+ "<span class='spanStyle' id='cardName"
						+ list[i][0].boatmanID + "'>" + list[i][2].cardName
						+ "</span>" + "<span class='spanStyle' id='boatmanCard"
						+ list[i][0].boatmanID + "'>" + list[i][0].boatCardNum
						+ "</span></li>");
...
function determineBoatInfo() {
	...
	for ( var i = 0; i < boatmanInfo.length; i++) {
		if (boatmanInfo[i][4] == 1) {
			...
			newTr.append($("<td><label>" + boatmanInfo[i][1]
					+ "</label><label>&nbsp;(&nbsp;" + boatmanInfo[i][2] + ":"
					+ boatmanInfo[i][3] + "&nbsp;)</label></td>"));
...
function showPortInfo() {
	$.ajax( {
		...
		dataType : 'json',
		data : {
			'shipName' : $("#boatName").val()
		},
		success : function(data) {
			var list = data.list;
			for ( var i = 0; i < list.length; i++) {
				var newLi_end = $("<li class='addLi' id='addPortLi"
						+ list[i].portID + "' onclick=addselectMUDIPort(this,'"
						+ list[i].portName + "','" + list[i].portID + "')>"
						+ list[i].portName + "</li>");
				$("#portname").append(newLi_end);
				var newLi_start = $("<li class='addLi' id='addStartPortLi"
						+ list[i].portID
						+ "' onclick=addselectStartPort(this,'"
						+ list[i].portName + "','" + list[i].portID + "')>"
						+ list[i].portName + "</li>");
...
function judgeWorkWharfshowOrhide() {
	...
	if (judgestartport !== "" || judgearriveport != "") {
		if (judgestartport.indexOf("湖州") >= 0
				|| judgearriveport.indexOf("湖州") >= 0) {
			$("#workwharf,#inOrNotPort").show();
		} else {
	...
	$.ajax( {
		...
		success : function(data) {
			...
			for ( var i = 0; i < list1.length; i++) {
				var newLi = $("<li class='addLi' onclick=showCargoType('"
						+ list1[i][0] + "')>" + list1[i][1] + "</li>");
				var newUl = $("<ul class='addUl' id='cargoul" + list1[i][0]
						+ "'></ul>")
			...
			for ( var j = 0; j < list2.length; j++) {
				var newLi = $("<li class='addLi' id='addCargoLi" + list2[j][0]
						+ "' onclick=addselectCargo(this,'" + list2[j][1]
						+ "','" + list2[j][0] + "')>" + list2[j][1] + "</li>");
				$("#cargoname").append(newLi);
	...
	$
			.ajax( {
				...
				success : function(data) {
					...
					for ( var i = 0; i < list.length; i++) {
						var newLi = $("<li class='addTypeLi' style='padding-left:45px;' id='addTypeLi"
								+ list[i].goodsID
								+ "' onclick=addselectTypeCargo(this,'"
								+ list[i].goodsName
								+ "','"
								+ list[i].goodsID
								+ "')>" + list[i].goodsName + "</li>");
...
function showPortList() {
	...
	$.ajax( {
		...
		success : function(data) {
			...
			for ( var i = 0; i < list.length; i++) {
				var newLi = $("<li class='addLi'  onclick=showAreaList('"
...
function showAreaList(id, name) {
	...
	$.ajax( {
		...
		success : function(data) {
			...
			for ( var i = 0; i < list.length; i++) {
				var newLi = $("<li class='addLi' onclick=showWharfInfo('"
						+ list[i].id + "','" + list[i].name + "')>"
						+ list[i].name + "</li>");
				$("#areaList").append(newLi);
			}
...
function showWharfInfo(id, name) {
	...
	$.ajax( {
		...
		success : function(data) {
			...
			for ( var i = 0; i < list.length; i++) {
				var newLi = $("<li class='addLi' id='addWharfLi" + list[i].id
						+ "' onclick=addselectWharf('" + list[i].id + "','"
						+ list[i].mc + "')>" + list[i].mc + "</li>");
				$("#wharfname").append(newLi);
			}
		}
	});
...
function selectWharfDiv(type) {
	$("#wharfname,#areaList,#portList,#PieceDiv,#WharfDiv").css("display",
			"none");
	if (type == 1) {
...
function addInfo() {
	...
	for ( var j = 0; j < selectboatmanInfo.length; j++) {
		if (selectboatmanInfo[j][4] == 1) {
			if (LinkshipInfo == "") {
				LinkshipInfo = selectboatmanInfo[j][0] + ","
						+ selectboatmanInfo[j][1] + ","
						+ selectboatmanInfo[j][2] + ","
						+ selectboatmanInfo[j][3];
			} else {
				LinkshipInfo += ";" + selectboatmanInfo[j][0] + ","
						+ selectboatmanInfo[j][1] + ","
						+ selectboatmanInfo[j][2] + ","
	...
	if ($("#reportId").val() != "null") {
		$.ajax( {
			url : 'UpdateElectricReport',
			type : 'post',
			dataType : 'json',
			data : {
				'electricReportNew.reportID' : $("#reportId").val(),
				// 'electricReportNew.shipUser':$("#shipusername").val(),
...
function checkform() {
	...
	if ($("#selectsail option:selected").val() == 1 && (startport.indexOf("湖州") >= 0 || arriveport.indexOf("湖州") >= 0)
			&& stopWharf == "") {
		alert("本次作业码头不能为空");
		return false;
	}
