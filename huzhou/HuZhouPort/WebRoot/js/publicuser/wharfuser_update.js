...
function wharfNameCheck() {
	var ret = true;
	ret = emptyChk("#wharfName", "#wharfNameerr", "绑定船名不能为空");
	if (ret == true) {
...
function deleteUpload(no,j,fileId) {
    ...
    if(fileId){
    	DeleteById("deleteWharfFileById","",fileId)
    }
}
function removeFile(no,shipId){
	...
	if(shipId){
		DeleteById("deleteWharfBindingById",shipId,"");
	}
}
function DeleteById(ationName,shipId,fileId){
	$.ajax( {
		...
		dataType : "json",
		data:{
		'wharfBinding.wharfId':shipId,
		'wharfFile.fileId':fileId
	},
		success : function(data) {
		append(data.list);
	},
...
function append(list){
	for(var no=0;no<list.length;no++){
		var tr=$("<tr class='addTr' id='shopFile_"+no+"'></tr>");
		tr.append("<td><input type='hidden' name='listw["+no+"].wharfId' value='"+list[no][5]+"'><input name='listw["+no+"].wharfNumber' value='"+list[no][1]+"' size='10'></td>");
		tr.append("<td><input name='listw["+no+"].wharfNum' value='"+list[no][0]+"'></td>");
		var cl=ConditionsArray.length;
		ConditionsArray[cl]=1;
		tr.append("<td><label onclick='showShopDialog("+cl+")' id='con"+cl+"' class='con' >满足条件</label></td>");
       var fileNameVar="";
       var fileIndex=0;
		if(list[no][3]!=null&&list[no][3]!=""){
        	var fileIdList=list[no][3].split(";");
		var fileNameList=list[no][2].split(";");	
		
		for(var j=0;j<fileIdList.length;j++){
			fileNameVar+="<div id='showFileName_"+no+"_"+j+"'><input type='hidden' name='listw["+no+"].listf["+j+"].fileId' value='"+fileIdList[j]+"'>"+fileNameList[j]+ "<a onclick='deleteUpload("+no+","+j+","+fileIdList[j]+")' " + "class='operation'>删除</a></div>";
		
		}
		var fileIndex=list[no][3].length;
        }
		tr.append("<td id='file"+no+"'><div class='showFileName' id='showFileName"+no+"'>"+fileNameVar+"</div></td>");
		tr.append($("<td><div class='file_class' id='file_class"+no+"'>" +
	"<img src='image/operation/add_attachments_click.png' class='file_img' title='添加附件' />" +"&nbsp;&nbsp;&nbsp;&nbsp;"+
	"<input type='file' onchange='appendFile("+no+","+fileIndex+")' class='file1_button' id='uploadFile"+no+"_"+fileIndex+"' name='listw["+no+"].listf["+fileIndex+"].file'/>" +
	"<input type='hidden' id='fileName"+no+"_"+fileIndex+"' name='listw["+no+"].listf["+fileIndex+"].fileName'></div>" +
						"<a class='operation' onclick=removeFile('shopFile_"+no+"','"+list[no][5]+"')>" +
						"删除</a></td>"));
		$("#usertable").append(tr);
	}
}