...
function showWharfwork(){
	$.ajax({
		...
		success :function(data){
			...
			WharfworkList(data.pagemodel,data.current);
		} 
	});
	
}

function WharfworkList(pagemodel,current){
    ...
    if(listLength==0){
		...
		newTr.append($("<td colspan=9>暂无相关数据</td>"));
		$("#wharfworkTable").append(newTr);
    }else{
    	$("#fenye").show();
    	for(var i=0;i<listLength; i++){
    		newTr=$("<tr class='clear'></tr>");
        newTr.append("<td>"+pagemodel.recordsDate[i].wharfID+"</td>");
        newTr.append("<td>"+pagemodel.recordsDate[i].wharfName+"</td>");
    	...
    	if(pagemodel.recordsDate[i].portMart=="1"){
        ...
        newTr.append("<td><a class='operation' href='"+basePath+"page/wharfwork/wharfworkshow.jsp?ID="+pagemodel.recordsDate[i].id+"'>查看</a></td>");
...
function selectContent(){
	...
	if(content==""){
		...
		$.ajax({
			url:'selectWharfWork1',
			type:"post",
			dataType: "json",
			data:{'content' : content},
			success :function(data){
				//alert("success");
				selectWharfWork(data.pagemodel,data.current,content)
			} 
		});
		
    }
}
function selectWharfWork(pagemodel,current,content){
	var basePath=$("#basePath").val()
	$(".clear").empty();
	$(".clear").remove();
    $("#fenye").hide();
    ...
    if(listLength==0){
        ...
        newTr.append("<td><a class='operation' href='"+basePath+"page/business/dangerousGoodsPortlnApproval.jsp?ID="+pagemodel.recordsDate[i].id+"'>查看</a></td>");
