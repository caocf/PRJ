$(document).ready(function() {
		showIllegalList("showIllegalList",1);// 待审核列表
		
});
//待审核列表
function showIllegalList(actionName, selectedPage){
/*	'queryCondition.listKeyValuePair[0].key' : '(il.enforecers1',
	'queryCondition.listKeyValuePair[0].pair' : '=',
	'queryCondition.listKeyValuePair[0].value' : id,
	'queryCondition.listKeyValuePair[0].connector' : 'or',
	'queryCondition.listKeyValuePair[1].key' : 'il.reviewUser',
	'queryCondition.listKeyValuePair[1].pair' : '=',
	'queryCondition.listKeyValuePair[1].value' : id,
	'queryCondition.listKeyValuePair[1].connector' : ')and',*/
	...
	$.ajax( {
		...
		data : {
			...
			'queryCondition.listKeyValuePair[0].connector' : 'and',
			'queryCondition.listKeyValuePair[1].key' : 'il.reviewUser',
			'queryCondition.listKeyValuePair[1].pair' : '=',
			'queryCondition.listKeyValuePair[1].value' : id,
...
function appendToTable(list, listLength){
	...
	for ( var i = 0; i < listLength; i++) {
		...
		newTr.append($("<td>"+GetShortTime(list[i][0].illegalTime)+"</td>"));
		if(list[i][0].reviewWether==0&&list[i][0].reviewUser==id)
		{
			newTr.append($("<td>待审核</td>"));	
			newTr.append($("<td><a onclick='GotoSee("+list[i][0].illegalId+")' class='operation'>查看</a>&nbsp;&nbsp;" +
					"<a onclick='GotoUpdate("+list[i][0].illegalId+")' class='operation'>补充</a>&nbsp;&nbsp;" +
						"<a onclick='GotoCheck("+list[i][0].illegalId+")' class='operation'>审核</a></td>"));
		}
		if(list[i][0].reviewWether==0&&list[i][0].reviewUser!=id)
		{
			newTr.append($("<td>待审核</td>"));	
		...
		if(list[i][0].reviewResult==2&&list[i][0].reviewUser==id){
