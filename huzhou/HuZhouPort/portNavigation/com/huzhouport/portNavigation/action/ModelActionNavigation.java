package com.huzhouport.portNavigation.action;
...
import com.huzhouport.portNavigation.model.Navigation;
import com.huzhouport.portNavigation.service.NavigationService;
import com.opensymphony.xwork2.ModelDriven;

@SuppressWarnings("serial")
public class ModelActionNavigation extends BaseAction implements
		ModelDriven<ModelPortNavigation> {
	ModelPortNavigation modelPortNavigation = new ModelPortNavigation();

	private List<ModelPortNavigation> list = new ArrayList<ModelPortNavigation>();// 返回列表	
	...
	public int pagesInfo() throws Exception {
		String pageUrl = "http://www.hzgh.gov.cn/hzgh/zwgk/tzgg/hxtg/";
		URL url;
...
//		try{
		...
		for (int i = 1; i < tiaoshu.length; i++) {
			ModelPortNavigation model = new ModelPortNavigation();
		...
		list=this.navigationService.findNavigationServiceInfo(modelPortNavigation,totalPage,  Integer.valueOf(this.getCpage()));
 		return SUCCESS;
	}
	...
	public String findDetailInfo() throws Exception {
		...
		String line;
		try{
		url = new URL(modelPortNavigation.getUrl());
		httpConn = (HttpURLConnection) url.openConnection();
		input = new InputStreamReader(httpConn.getInputStream(), "utf-8");
		bufReader = new BufferedReader(input);
...
//		  modelPortNavigation.setTitile(list1.get(0).replaceAll("<.*?>", "").replaceAll("&nbsp;", ""));// 去图片注释 
// 
...
//		  }   b.replaceAll("&nbsp;", "").replaceAll("font-size: 15pt", "font-size: 12pt").replaceAll("style.*?\"", "")
		...
		modelPortNavigation.setContenct(noStly.replaceAll("font-size: 15pt", "font-size:13pt").replaceAll("mso-line-height-rule: exactly", "").replaceAll("text-indent: 79.5pt", ""));
//				b.replaceAll(
...
//				.replaceAll("&nbsp;", "").replaceAll("text-indent.*?;", "text-indent:30pt;").replaceAll("line-height.*?;", "line-height:25pt;"));
	...
	public ModelPortNavigation getModel() {
		return modelPortNavigation;
	}

	public ModelPortNavigation getModelPortNavigation() {
		return modelPortNavigation;
	}

	public void setModelPortNavigation(ModelPortNavigation modelPortNavigation) {
		this.modelPortNavigation = modelPortNavigation;
	}

	public List<ModelPortNavigation> getList() {
		return list;
	}

	public void setList(List<ModelPortNavigation> list) {
		this.list = list;
	}
